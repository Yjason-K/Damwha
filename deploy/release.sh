#!/usr/bin/env bash
# Cut an install release: two arm64 images to GHCR, the worker wheel, and a
...
#
# Images, wheel and tarball share the tag so nobody can pair an API from
...
mkdir "$OUT/damwha"
cp deploy/docker-compose.yml deploy/.env.example deploy/Makefile \
   deploy/README.md deploy/HUGGINGFACE.md "$OUT/damwha/"
...
gh release create "v$VERSION" "$WHEEL" "$TAR" --target "$(git rev-parse HEAD)" \
  --title "v$VERSION" --notes "Self-hosted install. Setup: deploy/README.md inside the tarball."
