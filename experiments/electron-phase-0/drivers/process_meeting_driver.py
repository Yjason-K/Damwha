...
**Docker 를 띄운다** — Phase 0 이 제거하려는 바로 그 의존이고, 그 컨테이너
라이브러리는 `dev` 의존성 그룹이라 `models` extra 만 담은 번들에 존재하지도
않는다 (스펙 P0-C4). 그 파일은 **읽기 참조 전용이며 실행하지도 수정하지도
않았다.** 여기서 참고한 것은 페이로드의 모양과 시드 순서(회의 → audio_key →
job → current_job_id)뿐이다.

그 라이브러리의 **이름을 이 파일 어디에도 적지 않는다.** 계획 Task 7 의 V9 가
`grep -c '<그 이름>' <이 파일>` 이 `0` 이기를 요구하는데, 그것은 리터럴 문자열
개수를 세는 검사라 설명문에 이름이 한 번만 나와도 1 이 된다. 뜻은 이 문단이
그대로 담고 있다.
