...
export interface WindowFlowDeps {
  ...
  start(): Promise<void>;
  /**
   * 자동 재시도가 허용되는가 — 게이트 서비스의 실패가 manual이 아닌가
   * (main.ts: `mayAutoRetry(supervisor?.statuses() ?? null)`). 감독자가 아직 없으면(상태가
   * 없으면) 항상 true다 — retry-policy.ts의 `mayAutoRetry(null)` 규약과 같다.
   *
   * false면 창을 다시 열었다고 `start()`를 부르지 않는다 — manual 실패는 마이그레이션 게이트
   * 같은 사람 손이 필요한 실패라, 창을 여닫을 때마다 다시 걸면 20초 백오프가 아니라 창을 여는
   * 그 자리에서 게이트를 또 돌려 백업을 하나씩 쌓는다(스펙 §6.7). 셸은 이미 위에서 걸었으므로
   * 화면이 비지는 않는다 — 탈출구는 메뉴/화면의 "다시 시도" 버튼뿐이다.
   */
...
export async function openWindowFlow(deps: WindowFlowDeps): Promise<void> {
  ...
  if (!deps.readyToAttach()) {
    // 붙일 것이 없다. 세 경우다.
    //
    // (1) 게이트가 auto로 넘어진 채 창이 닫혔다 (Task 14). 창이 없으면 자동 재시도는 헛돌고(스폰
    // 가드가 창 없는 기동을 막는다) 걸려 있던 타이머도 다음을 걸지 못한다. 예전에는 여기서 붙이기로
    // 가 reattachWindow가 카운트다운 없는 실패 화면을 걸었다 — 거짓은 아니었지만 사용자가 돌아온
    // 순간 앱이 아무것도 하지 않았다. 기동으로 가면 재시도가 돌고, 또 넘어지면 **실제로 걸린**
    // 타이머의 초를 화면이 말한다. 화면의 카운트다운과 걸린 타이머가 늘 같은 자리(scheduleRetry)에서
    // 나온다.
    //
    ...
    // 메뉴의 "다시 시도"뿐이고 화면에는 그 안내가 없다 (재리뷰 §4-3). autoRetryAllowed()는 상태가
    // 없을 때 항상 true라(mayAutoRetry(null)과 같은 규약) 이 경우 아래에서 걸리지 않는다.
    //
    // (3) 게이트가 manual로 넘어졌다 (Task 12 fix round 1, 스펙 §6.7). 창을 다시 열었다고
    // start()를 부르면 마이그레이션 게이트 같은 사람 손이 필요한 실패가 창을 여닫을 때마다
    // 다시 돌아 백업을 하나씩 쌓는다. 셸은 이미 위에서 걸었으므로 화면은 비지 않는다.
    if (!deps.autoRetryAllowed()) return;
    await deps.start();
    return;
  }
  try {
