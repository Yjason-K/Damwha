 ...
 *
 * 0. 진입할 때 자손 집합을 한 번 찍어 둔다. 아래 어느 단계에서 "깨끗하다"고 말하기 전에
 *    그 집합이 아직 살아 있는지 되본다 — supervisor가 먼저 죽으면 그 자손은 pid 1로
 *    재부모화되어 사후 BFS로는 보이지 않는다.
...
export async function stopWorkerProcess(
  ...
  // 진입 스냅샷. supervisor가 아직 살아 있는 지금이 그 자손을 볼 수 있는 유일한 시점이다.
  ...
  const atEntry = await opts.descendants(pid).catch(() => new Set<number>());
  ...
  /** supervisor는 죽었다. 진입 때 붙잡아 둔 자손이 남아 있으면 깨끗한 종료가 아니다. */
  const cleanUnlessOrphans = async (): Promise<StopOutcome> => {
    const orphans = await survivors([...atEntry]);
    return { stopped: orphans.length === 0, leaked: orphans };
  };
  ...
  // 걸은 트리다 — 진입 스냅샷은 두 번의 유예만큼 낡아서, 그 사이 끝난 pid를 OS가 재사용했다면
  // SIGKILL이 남의 프로세스로 간다. 스냅샷은 "죽었나"를 읽는 데만 쓰고 죽이지는 않는다.
  const tree = await opts.descendants(pid).catch(() => new Set<number>());
  for (const target of [pid, ...tree]) opts.signal(target, "SIGKILL");
  await new Promise((r) => setTimeout(r, opts.pollMs));

  // 5단계. 진입 스냅샷도 후보에 넣는다 — 진입 때 우리 자손이었는데 끝까지 살아 있다면
  // 그 사이 부모를 잃어 BFS에서 사라졌더라도 여전히 우리가 남긴 프로세스다.
  const candidates = [...new Set([pid, ...tree, ...atEntry])];
