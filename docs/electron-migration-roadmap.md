...
하나는 디스크 부족(P5-C6)이고 Phase 6이 받는다. Phase 6은 스펙·계획도 미착수다.
...
- 업데이트 전 백업, DB 스키마·런타임 버전 호환성, 업데이트 실패 복구 정책. **백업·복원 기구 전체가
  Phase 5에서 넘어왔다** — Phase 3이 마이그레이션 직전에 뜨는 `pg_dump -Fc` 5개를 이미 돌리므로 절반은
  있다. 다른 맥으로의 데이터 이전이 필요해지면 여기서 다룬다.
- **디스크 부족의 원인·복구 안내 — Phase 5의 P5-C6이 넘어왔다.** `desktop/src/diagnostics/causes.ts`의
  ...
  스택트레이스)을 돌려주고 화면에는 아무 사유도 뜨지 않는 것을 확인했다. 남은 용량을 재는 자리와
  업로드의 ENOSPC 처리가 함께 필요하다.
