...
6b-3의 마이그레이션이 v1→v2 검증의 시험체라는 판단은 그대로다. **조회 원천의 전제 하나가
바뀌었다** — 웹 배포 퇴역(PR #28)으로 저장소 Latest가 데스크톱으로 넘어가 `/releases/latest`도 지금은
데스크톱을 가리키지만, 옛 `v*` 릴리스가 남아 있고 Latest는 손으로 바꿀 수 있는 값이라 `desktop-v`
접두사 필터를 유지한다.

**상태 (2026-09-23): 6b-1 스펙 작성·코덱스 리뷰 반영.** 스펙은
