...
**9건 충족·1건 Phase 6 이관·1건 미판정·1건 미실행**이다. 2026-09-20 화면 관측 라운드가 남아
있던 화면 절반을 실제로 보면서 P5-C9를 충족으로 올리고 P5-C8에서 결함 하나를 잡았다 — DB 장애가
`--once` 자식을 같이 죽이면 그 job이 30분 reaper까지 `running`으로 얼어붙는데 화면은 계속
"처리하고 있어요"라고 말했다(결과 문서 §5). **회수의 네 번째 층을 더해 고치고 같은 시나리오를
packaged로 다시 돌려 확인했다**(§6, `6303d53`). 미실행으로 남은 하나는 잠자기 10분(P5-C5)이고
사용자가 직접 돌린다. Phase 6은 스펙·계획도 미착수다.
...
거둔다. 스펙 완료 기준 12건 중 **9건 충족·1건 Phase 6 이관·1건 미판정·1건 미실행**이다.
