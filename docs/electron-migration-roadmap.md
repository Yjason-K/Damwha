...
**8건 충족·1건 미충족·1건 Phase 6 이관·1건 미판정·1건 미실행**이다. 2026-09-20 화면 관측
라운드가 남아 있던 화면 절반을 실제로 봤고(결과 문서 §5), 그 결과 P5-C9는 충족으로, P5-C8은
**미충족**으로 바뀌었다 — DB 장애가 `--once` 자식을 같이 죽이면 그 job이 30분 reaper까지
`running`으로 얼어붙는데 화면은 계속 "처리하고 있어요"라고 말한다. 미실행으로 남은 하나는
잠자기 10분(P5-C5)이고 사용자가 직접 돌린다. Phase 6은 스펙·계획도 미착수다.
...
거둔다. 스펙 완료 기준 12건 중 **8건 충족·1건 미충족·1건 Phase 6 이관·1건 미판정·1건 미실행**이다.
...
| 강제 종료·잠자기·디스크 부족·서비스 장애에서 원인과 복구 방법을 앱에서 확인 가능하고, 기존 회의 기록과 녹음 파일이 보존됨 | **부분 충족** | 보존은 확인됐다 — 디스크가 실제로 찬 회차에서 기존 `meeting`·`utterance` 행과 저장소 파일이 그대로였고 고아 DB 행도 생기지 않았다. 녹음 중 `kill -9`도 확정 경계 뒤 꼬리만 잃었다(P5-C9). **원인 문구는 화면 관측 라운드에서 실제로 확인했다** — embed·postgres 장애가 `프로세스가 종료됐어요 (코드 143/0)`와 해결 안내로 뜨고, API가 `데이터베이스에 연결할 수 없어요`로 연쇄까지 말한다. 그러나 **같은 장애가 `--once` 자식을 죽이면 job이 30분 reaper까지 `running`으로 얼어붙고 화면이 "처리하고 있어요 35%"라는 거짓을 말한다** — 그래서 P5-C8은 미충족이다(결과 문서 §5). 디스크 부족은 이 코드가 아직 사유를 만들지 않아 Phase 6으로 넘겼다(P5-C6). 3분 끊김(P5-C4)은 `/etc/hosts` 권한이 없어 미판정, 잠자기(P5-C5)는 미실행 |
...
| P5-C4 | 3분 끊김 뒤 같은 job이 사람 개입 없이 완주한다 | **미판정** — `/etc/hosts` 쓰기 권한이 없어 차단 자체를 못 했다 |
| P5-C5 | 10분 잠자기·복귀 후 60초 안에 job이 정착한다 | **미실행** — `pmset sleepnow`가 검증 세션 자신을 재운다. 사용자가 직접 돌린다 |
...
| P5-C8 | 처리 중 embed·DB 장애가 화면에 사유로 뜨고 60초 안에 회복된다 | **미충족** — 화면 문구와 60초 회복(embed 26초·postgres 8초)은 확인했으나, 같은 장애가 `--once` 자식을 죽이면 job이 `running`으로 얼어붙어 "진행 또는 재시도 대기로 정직하다"를 못 채운다 |
...
**이 Phase에 남은 결함이 하나다 — P5-C8.** DB가 죽으면 그 job을 쥔 `--once` 자식이 같이 죽는데,
supervisor 부모는 재연결만 하고 살아남으므로 회수 세 층 중 어느 것도 걸리지 않는다(기동 회수는 앞
실행의 행만 보고, `--once` 스캔은 supervisor가 재시작해야 돈다). 남는 것은 30분 reaper뿐이라 그동안
화면이 "처리하고 있어요"를 계속 말한다. 필요한 것은 네 번째 경로 — **DB 재연결 직후, 같은
`WORKER_ID`가 쥔 `running` 행 중 살아 있는 `--once` 자식이 없는 것을 되돌린다.** 결과 문서 §5에
실측과 근거가 있다.
