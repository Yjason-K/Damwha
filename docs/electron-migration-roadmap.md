...
**7건 충족·1건 부분 충족·1건 Phase 6 이관·1건 미판정·2건 미실행**이다. 미실행 둘(잠자기 10분,
녹음 중 강제 종료)은 이 세션에서 돌릴 수 없어 사용자가 직접 돌린다. Phase 6은 스펙·계획도 미착수다.
...
거둔다. 스펙 완료 기준 12건 중 **7건 충족·1건 부분 충족·1건 Phase 6 이관·1건 미판정·2건 미실행**이다.
...
| 강제 종료·잠자기·디스크 부족·서비스 장애에서 원인과 복구 방법을 앱에서 확인 가능하고, 기존 회의 기록과 녹음 파일이 보존됨 | **부분 충족** | 보존은 확인됐다 — 디스크가 실제로 찬 회차에서 기존 `meeting`·`utterance` 행과 저장소 파일이 그대로였고 고아 DB 행도 생기지 않았다. **화면 문구는 어느 회차에서도 보지 못했다** — 검증 세션에 화면 제어 권한이 없었다. embed·postgres 장애의 회복은 로그와 `/api/health`로만 확인했고(P5-C8 부분 충족), 디스크 부족은 이 코드가 아직 사유를 만들지 않아 Phase 6으로 넘겼다(P5-C6). 3분 끊김(P5-C4)은 `/etc/hosts` 권한이 없어 미판정, 잠자기(P5-C5)와 녹음 중 강제 종료(P5-C9)는 미실행 |
...
| P5-C8 | 처리 중 embed·DB 장애가 화면에 사유로 뜨고 60초 안에 회복된다 | **부분 충족** — 회복은 로그·`/api/health`로 확인, 화면 문구 미관측 |
| P5-C9 | 녹음 중 강제 종료의 손실이 확정 경계 뒤 꼬리로 한정된다 | **미실행** — 마이크가 필요하다. 사용자가 직접 돌린다 |
