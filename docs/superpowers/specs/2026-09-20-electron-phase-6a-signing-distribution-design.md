...
- **태그는 `v<version>`이다.** `package.mjs`가 `--release` 빌드에서
  `git describe --tags --exact-match`로 현재 커밋의 태그를 읽어 `v${pkg.version}`과 다르면
  **거절한다.**
...
`package.mjs`의 서명 뒤, `check-bundle` 앞에 넣는다. **`--release`일 때만 돈다**(§4).

1. **DMG 생성** — `electron-builder.yml`의 `mac.target`을 `dmg`로 두고, `--release`가 아닐 때만
   `--dir`을 붙여 부른다. 수 GB 번들의 DMG 압축은 분 단위라 개발 루프에 매번 물리지 않는다.
2. **공증** — `xcrun notarytool submit <dmg> --keychain-profile damwha --wait`.
   - 프로필 이름 `damwha`도 `signing.json`에 둔다.
   - 실패하면 `notarytool log <id>`를 그대로 출력하고 멈춘다. 공증 거절 사유는 목록이라
     요약하면 원인을 잃는다.
   - 타임아웃 상한을 둔다(기본 대기는 무한). 30분으로 하고 넘으면 제출 id를 남기고 멈춘다 —
     제출은 살아 있으므로 나중에 `notarytool log`로 이어 볼 수 있다.
3. **스테이플** — `xcrun stapler staple <dmg>`. `.app`에도 따로 건다(DMG 밖으로 꺼낸 `.app`이
   오프라인에서도 통과하게).
4. **검증** — `spctl --assess --type execute -vv <app>`이 `accepted / source=Notarized Developer ID`를
   내는지, `stapler validate`가 통과하는지. `check-bundle.mjs`가 이 둘을 단언으로 받는다.
...
모델 한 벌이 수 GB라 여기가 주 사고 지점이다. worker가 다운로드를 시작하기 전에 목적지
볼륨의 여유를 재고, **필요한 용량보다 적으면 받지 않고 실패한다.**

- 필요한 용량은 HF API가 주는 저장소 크기에 여유 계수를 곱한다. 값을 못 얻으면 이 점검을
  건너뛴다(추정으로 막지 않는다 — 막는 쪽이 틀리면 받을 수 있는 것을 못 받는다).
- 실패는 **PERMANENT**다. 디스크가 그대로인 채 재시도해 봐야 같은 자리에서 진다.
  Phase 5가 만든 백오프 5회를 디스크 부족으로 태우지 않는다.
- 사유 문자열은 `causes.ts`의 `diskFull`이 이미 정한 모양(`남은 용량 X, 필요한 용량 Y`)을 쓴다.
  **문구를 새로 만들지 않는다** — 그 파일의 머리 주석이 금하는 것이다.
...
| ID | 기준 | 환경 | 확인 방법 |
| --- | --- | --- | --- |
| P6a-C1 | 번들 Mach-O 전수의 `minos` 최대값이 15.0 이하다 | static | `vtool -show-build` 전수. 초과 파일 0개. 실패 시 경로·값 전부 보고 |
...
| P6a-C6 | 디스크 부족에서 원인과 복구 안내가 화면에 뜬다 | packaged | §11-2의 주입 절차. 모델 다운로드 앞(8.1)과 업로드(8.2) **양쪽** 관찰. 화면에 `디스크 공간이 부족해요 — 남은 용량 …, 필요한 용량 …`과 안내 문구 |
| P6a-C7 | 디스크 부족 회차 뒤 기존 데이터가 보존되고 고아 행이 없다 | packaged | 전후 `meeting`·`utterance` 행 수와 `data/storage` 파일 수가 같다. 업로드 실패가 `meeting` 행을 남기지 않는다. Phase 5의 정합성 질의 넷이 0행 |
| P6a-C8 | 디스크 부족이 재시도 예산을 태우지 않는다 | packaged | 해당 job의 `attempts`가 1 증가에 그치고 `failed`. `queued`로 돌아 백오프를 도는 일이 없다 |
...
1. **6b의 알림 원천은 `GET https://api.github.com/repos/Yjason-K/Damwha/releases/latest`로
   정했다**(저장소가 public임을 확인). 6a에서 할 일은 없고, 6a가 내는 첫 릴리스가 그 API의
   첫 응답이 된다는 것만 기록해 둔다 — **6a의 배포는 GitHub Release로 낸다.**
