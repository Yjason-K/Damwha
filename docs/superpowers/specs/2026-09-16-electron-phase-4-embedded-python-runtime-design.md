...
#### 6.6-b 오프라인 적재와 무진행 감시 (2026-09-18 추가)

**이 절은 Part 2 Task 9의 실측이 요구해서 생겼다.** 초안은 "캐시가 차 있으면 네트워크가
없어도 적재된다"를 `huggingface_hub`의 기본 동작으로 전제했는데 **그 전제가 세 곳에서
거짓이었다** (근거·명령·출력은 결과 문서의 Task 9 절에 있다. 라이브러리는
`huggingface_hub 1.20.1` + `hf-xet 1.5.1`):

1. **`SentenceTransformer`(bge-m3)는 네트워크가 끊기면 캐시가 차 있어도 실패한다** —
   `utils/_http.py:436`이 공유 클라이언트를 루프 밖에서 잡고 `ConnectError`에
   `close_session()`(:486) 뒤 같은 객체로 재시도해 `RuntimeError: client has been closed`가
   나고, 그 타입은 hub·transformers의 캐시 폴백 `except`에 걸리지 않는다. 발화 조건은
   "캐시에 없는 파일 + `ConnectError`"이고, transformers가 저장소에 없는 선택 파일
   (`adapter_config.json`)을 매번 묻기 때문에 정상 경로에서 발화한다.
2. **패킷이 버려지는 네트워크(먹통·캡티브 포털)에서는 `snapshot_download` 계열
   (mlx-whisper·mlx-lm·faster-whisper)이 캐시가 차 있어도 무한 대기한다** — 공유 클라이언트가
   `timeout=None`이고(`utils/_http.py:283-287`) `repo_info`에 `etag_timeout` 같은 방어가 없다.
3. **끊긴 xet 다운로드는 오류 없이 멈춘다** — 네트워크가 돌아와도 재연결 시도가 없고
   600초 예산을 그대로 소진했다. 그동안 `--once` 자식은 heartbeat 때문에 살아 있는 것으로
   보여 어느 회수 경로도 그것을 거두지 않는다.

그래서 이 Phase는 세 가지를 **명시적으로** 만든다:

- **캐시 우선 적재.** 모델 적재는 `local_files_only`(또는 각 로더의 동등물)로 **먼저** 시도하고,
  실패했을 때만 네트워크로 내려간다. 실측: 실 2.1 GB 캐시에서 bge-m3가 오프라인 1.1초에 적재된다.
  덤으로 P4-C9의 "리비전 확인 요청"도 사라진다.
- **모든 HF 요청에 상한을 준다.** 메타데이터·다운로드·xet 각각에 유한한 타임아웃을 주어
  "먹통 네트워크"가 무한 대기가 되지 않게 한다. 상한을 넘으면 TRANSIENT 실패다.
- **무진행 감시.** worker의 다운로드에도 §6.9의 무진행 규칙을 적용한다 — 진행이 멈춘 채
  제한을 넘으면 그 다운로드를 TRANSIENT 실패로 끝내 job을 큐로 돌려보내고 자식이 끝나게 한다.
  감시 대상은 embed·LLM만이 아니라 `--once` 자식의 다운로드를 포함한다.

**바이트 단위 이어받기는 이 Phase의 범위가 아니다** — 아래 §9 P4-C7의 각주를 보라.
...
| P4-C7 | 다운로드를 끊으면 원인이 뜨고, 회복 뒤 **다음 job이 다시 받아 완주한다** | 다운로드 중 네트워크 차단 → 화면에 사유(무진행 감시가 유한 시간에 끝낸다) → 복구 → 다음 job이 그 모델을 받아 처리를 마친다. **바이트 단위 이어받기는 요구하지 않는다** — 아래 각주 |
...
| P4-C29 | 모델을 받아 둔 뒤에는 네트워크 없이도 처리가 된다 | 캐시가 찬 상태에서 네트워크를 끊고 업로드 1건 완주. **검색·색인(bge-m3)을 포함한다** — §6.6-b의 캐시 우선 적재가 그것을 참으로 만든다 |
...
**P4-C7 각주 — 왜 "이어받기"에서 "다시 받아 완주"로 좁혔나 (2026-09-18, 사용자 결정).**
초안은 "복구 뒤 다음 job이 **처음부터가 아니라** 이어받음"을 요구했다. Part 2 Task 9가
실측으로 그것이 **현재 라이브러리로 만들 수 없음**을 확정했다 — `huggingface_hub 1.20.1`은
받을 곳을 프로세스마다 다른 이름(`<etag>.<uuid8>.incomplete`)으로 만들고 실패하면 지우므로
**프로세스 사이 resume이 존재하지 않고**(같은 PR 주석이 깨진 `flock` 때문에 공유
`.incomplete`를 포기했다고 적는다), 프로세스 안 재개도 "1초 안에 복구된 끊김" 한 번뿐이다.
xet 경로는 재개가 없을 뿐 아니라 끊기면 오류 없이 멈춘다(§6.6-b 3번). 훅은 이 동작에 영향이
없다 — 훅 유무로 재다운로드량이 바이트 단위로 같았다.

이어받기를 만들려면 (i) xet을 끄고 hub를 패치해 공유 `.incomplete`를 되살리거나 (ii) 자체
다운로더를 쓰는 길뿐이고, **둘 다 이 Phase의 목표(실행 환경 내장)를 넘어선다.** 그래서 기준을
사용자가 관찰하는 결과("끊기면 사유가 뜨고, 네트워크가 돌아오면 다음 job이 완주한다")로
좁히고, 바이트 재사용은 후속 Phase의 개선 항목으로 §15에 남긴다. 좁힌 기준은 §6.6-b의
무진행 감시가 있어야 성립한다 — 그것이 없으면 끊긴 다운로드가 사유를 내지 못하고 무한히 멈춘다.
...
| **R4-15** | **끊긴 다운로드가 `--once` 자식을 무기한 붙잡는다** (xet은 오류 없이 멈추고 heartbeat는 계속 뛰어 어느 회수 경로도 그것을 안 거둔다 — 2026-09-18 실측) | §6.6-b의 유한 타임아웃 + worker 다운로드 무진행 감시. 넘으면 TRANSIENT 실패로 끝내 job을 큐로 돌려보낸다 |
| **R4-16** | **캐시가 차 있는데도 오프라인·먹통 네트워크에서 적재가 실패하거나 멈춘다** (hub 1.20.1의 닫힌 클라이언트 재사용, `timeout=None` 공유 세션 — 2026-09-18 실측) | §6.6-b의 캐시 우선 적재. P4-C29가 검색·색인까지 포함해 판정한다 |
...
- 중단된 다운로드의 부분 파일 위생 (HF hub의 `.incomplete`). **2026-09-18 실측:**
  SIGKILL로 끊긴 다운로드는 `<etag>.<uuid8>.incomplete`를 남기는데 hub가 그것을 **다시 쓰지
  않으므로**(프로세스마다 이름이 다르다) 성공한 뒤에도 고아로 남는다 — 위생 대상이 실재한다.
- **바이트 단위 이어받기** — P4-C7을 "다시 받아 완주"로 좁히면서 여기로 넘겼다(§9 P4-C7 각주).
  만들려면 xet을 끄고 hub를 패치하거나 자체 다운로더를 써야 한다. 대가는 중복 대역폭뿐이고
  기능 실패는 아니다.
...
1~4회차가 본 텍스트와 지금의 Part 2는 같은 문서가 아니다. 분할 뒤 Part 2 파일은 2커밋에서
21줄(+20/-1)만 바뀌었고 그것도 Part 1 리뷰의 파급이었다.
...
| BL-6 | P4-C12의 다섯 자기 보고 중 **둘이 안 만들어지고 셋째는 삼켜진다** | `llm_entry`의 보고를 만드는 Step이 없다. `capabilities.py:67-77`이 `r.stderr`를 아무도 읽지 않는다. `embed_service.py:39`의 `main()`에 `install_logging()`이 없다. 계획이 적은 근거("부모가 stdout을 JSON으로 파싱한다")도 사실이 아니다 — 실제는 `{"1": True, "0": False}.get(r.stdout.strip())` | 다섯을 **어느 Task·Step이 만드는지 표로** 못 박고 연결표 C12를 `2·3`으로 |
...
### 17.12 스펙 개정 — Part 2 Task 9의 실측 (2026-09-18, 사용자 승인)

**대상: 이 스펙 (HEAD `b3a3ad9`).** 8회차까지의 검증은 계획·스펙 텍스트를 읽은 것이고, 이
개정은 **구현이 실행으로 반증한 전제** 하나에서 나왔다. Part 2 계획 말미의 "구현 중 판정할 것"
1번이 지정한 자리(Task 9 Step 6)에서 판정했고, 그 항목이 예고한 대로 **"아니면 그것을 만드는
Task가 필요하다"에 해당했다.**

| # | 반증된 전제 | 실측 | 이 개정 |
| --- | --- | --- | --- |
| 1 | "훅이 hub의 resume 경로를 깨뜨릴 수 있다"(확인 필요 항목) | **훅은 무죄** — 훅 유무로 재다운로드량이 같다(HTTP 353.3 MB, xet 312.6 MB) | 없음(기록) |
| 2 | "끊긴 다운로드는 다음에 이어받는다"(P4-C7) | hub 1.20.1에 **프로세스 간 resume이 없다**(프로세스별 `.incomplete` + 실패 시 삭제). xet은 끊기면 오류 없이 600초+ 멈춘다 | **§9 P4-C7을 "다시 받아 완주"로 좁히고 각주에 근거.** 바이트 이어받기는 §15 Phase 5로 |
| 3 | "캐시가 차 있으면 오프라인에서 적재된다"(P4-C29) | `hf_hub_download` 경로는 참. **bge-m3(SentenceTransformer)는 hub의 닫힌 클라이언트 재사용 버그로 실패**. 먹통 네트워크에서는 `snapshot_download` 계열이 **무한 대기** | **§6.6-b 신설**(캐시 우선 적재·유한 타임아웃·무진행 감시). §9 P4-C29가 검색·색인까지 포함함을 명시 |
| 4 | (새로 드러남) 끊긴 다운로드가 `--once` 자식을 무기한 붙잡는다 | heartbeat가 뛰므로 고아 판정에도 안 걸린다 | **§13 R4-15·R4-16 추가.** §6.6-b의 무진행 감시가 막는다 |

**계획에 미치는 영향:** §6.6-b를 구현하는 Task가 Part 2 계획에 없다. 확정된 계획 문서는
보존하므로(로드맵 §6) **컨트롤러가 Task 9b를 SDD 작업공간의 브리프로 추가**하고 Task 10 앞에
실행한다. 그 사실과 근거는 결과 문서에 남는다.

**실측 환경:** 시스템 네트워크를 건드리지 않기 위해 CONNECT 프록시로 `pass`/`reset`/`blackhole`
세 상태를 만들고, `HF_HOME`을 스크래치로 돌린 채 공개 저장소만 썼다. 그 회차가
`~/.cache/huggingface/xet/logs/`에 로그 1개(1001 B)를 남겼고 — hf_xet의 로그 경로는 `HF_HOME`을
따르지 않는다 — **사용자 결정으로 그 한 파일만 지워** 기준선을 바이트 단위로 되돌렸다(8건 PASS).
