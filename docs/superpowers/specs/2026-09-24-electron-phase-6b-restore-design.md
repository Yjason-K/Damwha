...
- **모든 postgres launch**: postgres 어댑터 `launch()`의 맨 앞에서 부르는 `preLaunch` 훅. "다시 시도"
  ...
  훅은 저널을 잇지 않는다(아래 항목) — 보류 대화상자는 첫 기동의 명시 호출만 띄운다. 첫 기동에서는 명시 호출 뒤
  훅이 한 번 더 도는데, 이미 기록이 있어 비용이 없다.
- **저널을 잇는 것은 첫 기동의 명시 호출뿐이다**(최종 리뷰 2026-09-24, §16.4). 훅은 가드를
  `journal: "refuse"`로 돌린다 — 저널(읽을 수 없는 것 포함)을 만나면 락 정리·파일 조작 **전에**
  `restorePending`(`manual`, "앱을 종료한 뒤 다시 열어 주세요")으로 거부한다. postgres 자동 재시작
  (restart maxAttempts 3)·상태 창 재시작은 종료 흐름의 `commit`이 저널을 쓴 뒤 `stopAll` 전에도 올 수 있고,
  그때 API·worker는 떠 있다 — 거기서 교체하면 §3.1("앱이 떠 있는 채로 디렉터리를 바꾸지 않는다")을 어긴다.
  명시 호출은 `journal: "advance"`로 §6.3을 끝까지 잇는다.
...
2. **쓰는 주체 부재 확인**: `reapBeforeStart`가 성공을 돌려도 SIGKILL 뒤 생존자를 로그로만 남긴다
   ...
   구현 메모: 이 재스캔은 가드 안이 아니라 `reapBeforeStart`에 있다 — 첫 기동의 가드 **바로 앞**에서 돈다.
   감독자가 살아 있는 재시도(훅 경로)는 이전 실행의 고아를 새로 만들 수 없으므로 거기서는 다시 보지 않는다.
...
- 늘 보인다. 비활성 조건: 외부 DB 모드 / §6.1을 만족하는 스냅샷이 없음 / 저널이 이미 있음. 활성·비활성은
  기동·데이터 가드 뒤에 판정하고 상태가 바뀌면 메뉴를 다시 세운다(Electron 메뉴는 정적이다). 가드가 거부로
  끝나도 다시 세운다(첫 기동·훅 둘 다 `finally`).
- dev에서도 메뉴는 packaged가 같은 `userData`에 뜬 스냅샷을 되돌리기 후보로 쓸 수 있고, 되돌리기가 끝나면
  `markRestored`가 `.damwha-generation`을 쓴다 — 사용자가 고른 동작이라서다. dev는 여전히 스냅샷을 뜨지
  않는다(C9).
...
| C9 | 외부 DB 모드·스냅샷 없음·저널 있음에서 메뉴 비활성, dev는 스냅샷·기록을 만들지 않는다(주: dev에서도 메뉴는 packaged가 뜬 스냅샷을 쓸 수 있고, 사용자가 고른 되돌리기의 `markRestored`는 `.damwha-generation`을 쓴다 — dev가 스냅샷을 뜨지 않는 것은 그대로다, §7.1) | 단위 |
...
- 앱 밖의 쓰는 주체(터미널 `pnpm worker`·`pnpm dev`의 API)는 데이터 가드가 모른다(`DAMWHA_MODULES`는
  ...
  따른다.
- **같은 판 재설치는 새 스냅샷을 뜨지 않는다.** 어떤 판의 첫 기동이 세대를 기록한 뒤 그 판의 마이그레이션이
  실패하고, 사용자가 메뉴 없이 손으로 이전 판을 다시 설치해 쓰다가 나중에 **같은** 판을 다시 설치하면,
  기록된 빌드가 같아 스냅샷이 뜨지 않는다. 그 업그레이드는 게이트의 새 마이그레이션 전 덤프(고정 아님)만
  지킨다. 메뉴로 되돌리거나(`restoredFrom`이 기록을 바꾼다), 다른 판을 설치하면 피한다.
...
### 16.4 최종 whole-branch 리뷰 (2026-09-24)

Important 두 건과 마이너 몇 건. 모두 반영했다.

| # | 지적 | 반영 |
| --- | --- | --- |
| I1 | postgres `preLaunch` 훅이 가드를 통째로 돌려 저널을 이었다 — 종료 흐름의 `commit` 뒤 `stopAll` 전 자동 재시작이 API·worker가 떠 있는 채로 `data/`를 바꾸고, `ctx.signal` 중단이 `requested` clone을 끊으면 저널을 지웠다 | §5.2 `journal: "advance" \| "refuse"`. 훅은 `refuse` — 저널을 만나면 아무것도 건드리지 않고 `restorePending`으로 거부. 첫 기동만 `advance` |
| I2 | `docs/RESTORE.md` 수동 절차가 `.damwha-generation`을 그대로 둬, 같은 새 판 재설치 때 `findUnrecorded`가 옛 스냅샷을 복원점으로 재사용했다 | 수동 절차 §3에 `restoredFrom: "manual-<시각>"` 기록 한 줄. 테스트가 문서의 그 줄을 실행한다 |
| m | 가드 거부 시 메뉴 미갱신·낡은 `lastStartFailure`·가드 중 종료 뒤 감독자 생성·시계가 뒤로 간 경우 방금 뜬 스냅샷 정리·떨어진 주석 | §7.1 `finally` 갱신, 실패 기록 덮어쓰기, 가드 뒤 `quitting`/세대 확인, 정리가 새 스냅샷 id를 보호 |
