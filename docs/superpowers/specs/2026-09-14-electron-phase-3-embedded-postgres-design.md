...
4. **마이그레이션 실행 게이트 계약** — 상태 조회·백업·실행·안전망, 그리고 자동 재시도를 막는 원인 부류.
...
| Phase 2 결과 §5.2-1 | 게이트 실패 뒤 창이 상한 없이 20초마다 자동 재시도 | §6.7의 원인 부류 `needs-action`. 실행 게이트가 생기면 이 루프는 마이그레이션·백업을 20초마다 반복하게 되므로 **반드시** 같이 고친다 |
...
| `desktop/src/main.ts` | `dockerRun`, docker 실행 파일 탐색, `scheduleRetry` 무조건 자동 재시도 | docker 경로 삭제. 자동 재시도가 원인 부류를 본다(§6.7) |
...
- 자동 재시도의 원인 부류(`transient` / `needs-action`) — Phase 2 §5.2-1 해소.
...
<userData>/
  data/                     ← 내장 모드의 "한 쌍". 같이 생기고 같이 검증된다
    ...
    storage/                STORAGE_ROOT
      .damwha-cluster       이 스토리지가 속한 클러스터의 system identifier
...
**마커.** `data/storage/.damwha-cluster`에 클러스터의 **system identifier**를 적는다. 값은 번들
`pg_controldata -D <PGDATA>`의 `Database system identifier` 줄에서 읽는다 — 서버를 띄우거나 DB에
접속하지 않고 읽히므로 판정이 기동보다 먼저 올 수 있다. `initdb`마다 새 값이 생기므로 "같은 경로에
다시 만든 클러스터"도 구별된다.

**"비어 있다"의 정의.** 스토리지 디렉터리가 없거나, 마커와 `.DS_Store` 말고는 항목이 없다.

**판정표** — 기동마다 서버를 띄우기 전에 한다.

| PGDATA | 스토리지 | 동작 |
| --- | --- | --- |
| 없음 | 비어 있음 | `initdb` → 마커 기록 (§6.4) |
| 있음 | 마커 = 클러스터 id | 기동 |
| 있음 | 비어 있음, 마커 없음 | 마커를 기록하고 기동 — `initdb` 뒤 마커를 쓰기 전에 중단된 경우 |
| 없음 | 비어 있지 않음 | **거부** |
| 있음 | 마커 ≠ 클러스터 id | **거부** |
| 있음 | 비어 있지 않음, 마커 없음 | **거부** |
| `PGDATA/PG_VERSION` ≠ 번들 메이저(`16`) | — | **거부** (메이저 업그레이드는 Phase 6) |
| `pg_controldata`가 실패 | — | **거부** (클러스터를 읽을 수 없다) |

거부는 원인·두 경로·"폴더를 옮기거나 지우지 않았는지 확인하세요"를 보여주고 아무것도 바꾸지 않는다.
원인 부류는 `needs-action`이다(§6.7).
...
바이트 수를 보여주고 거부한다(`needs-action`). 대체 디렉터리로 폴백하지 않는다 — `/tmp` 류는 다른
...
`ServiceSpec` 계약은 **바꾸지 않는다.** `postgres` 어댑터의 `detectExternal`·`launch`·`readiness`·`stop`·
`restart`만 바꾼다. `specs.ts`의 선언 순서(`postgres → api → embed → worker`)가 이미 postgres를
**맨 먼저 기동, 맨 나중에 종료**로 만든다(`supervisor.ts` `stopAll`의 `reverse()`).

| 필드 | Phase 2 | Phase 3 내장 모드 |
| --- | --- | --- |
| `gate` | `true` | `true` |
| `detectExternal` | compose 컨테이너가 healthy면 `adopt` | 항상 `absent`. 이전 실행의 고아는 외부가 아니라 `launch()`가 처리한다 |
| `launch` | `compose up -d`, 핸들 `null` | 아래 1~5, 핸들은 postmaster 자식 |
| `readiness` | compose `Health` | `postmaster.pid` 상태 (아래) |
...
2. **고아·낡은 락 판정** — `PGDATA/postmaster.pid`가 있으면 첫 줄의 pid를 `ps`로 확인한다.
   ...
   - `ps`가 실패하거나 명령을 읽지 못한다 → **증명할 수 없다.** 지우지 않고 거부한다(`needs-action`).
   - 고아가 fast 종료 유예 안에 끝나지 않으면 SIGQUIT, 그래도 남으면 거부(`needs-action`, pid 표시).
3. **`initdb`가 필요하면**(§6.2 첫 행) `data/postgres.initdb-<난수>`에 만든 뒤 `data/postgres`로
   `rename`한다. 중단된 `initdb`가 반쯤 만든 클러스터를 정상 PGDATA로 오인하지 않게 한다. 이전
   실행이 남긴 `postgres.initdb-*`는 앱이 만든 것이므로 지운다.
   인자: `-D <tmp> -U damwha --encoding=UTF8 --locale=C --auth-local=trust --auth-host=reject`.
...
5. **ready 뒤**(`readiness`가 처음 `ready`를 낸 직후, 게이트를 열기 전) 번들 `psql -h <run> -U damwha
   -d postgres`로 `damwha` 데이터베이스의 존재를 확인하고 없으면 번들 `createdb`로 만든다. 그다음
   마커가 없으면 기록한다. 매 기동 확인하는 이유: `initdb`와 `createdb` 사이에서 중단된 클러스터를
   구제한다. 이 단계의 실패는 `failed`(원인 표시)다.

   5단계를 어디서 부르는지(어댑터 `readiness`의 첫 `ready` 분기 안인가, 감독자가 부르는 선택 훅인가)는
   계획이 정한다. 감독자 계약을 바꾸는 쪽이면 그 이유를 계획에 적는다.

#### 준비 판정 (`readiness`)

프로세스를 스폰하지 않고 **파일로** 판정한다. PostgreSQL 10+는 `postmaster.pid` 8번째 줄에 상태를
적고(`starting` / `stopping` / `ready` / `standby`), `pg_ctl`이 기동 대기에 쓰는 신호가 바로 이것이다.
...
새 스키마에서 도는 것을 막는다. 원인 부류 `needs-action`. dev에서 브랜치를 오가면 걸리는 경우이므로
...
1. 번들 `pg_dump -h <run> -U damwha -Fc -f <backups>/<UTC>-before-<첫 pending 이름>.dump.partial damwha`.
2. 번들 `pg_restore --list <파일>`이 exit 0이면 `.partial`을 뗀다. 목차를 읽을 수 있음까지를 "검증"으로
   정의한다 — 복원 리허설은 Phase 5.
3. 성공한 뒤에만 보관 상한(최근 5개)을 넘는 가장 오래된 `.dump`를 지운다. `.partial`은 다음 백업 전에
   지운다.
4. 어느 단계든 실패하면(디스크 부족 포함) **마이그레이션하지 않고** 멈춘다. 원인 부류 `needs-action`.
...
기존 그대로(`migrate.ts:31-45`) — 실패한 파일은 롤백되고 앞서 성공한 파일은 남는다. 실패하면:

- API·worker를 띄우지 않는다.
- stderr의 원인 블록, 백업 경로(있으면), 로그 경로를 보여준다.
- 원인 부류 `needs-action`. 자동 재시도로 같은 마이그레이션을 반복하지 않는다.
...
- 내장 모드의 `DATABASE_URL`·`STORAGE_ROOT`는 **앱 소유 키**다. Phase 2의 `withAppOwned`처럼 파일 값보다
  **뒤에** 얹는다.
- `DEBUG_EXTERNAL_DATABASE_URL`·`DATABASE_URL`·`STORAGE_ROOT`는 `RESTART_ONLY_KEYS`에 더한다(§6.1 모드).
- 주입 대상은 Phase 2와 같다 — `DATABASE_URL`은 api·worker, 그리고 이 Phase의 `migrate.js`(두 호출
  모두). `STORAGE_ROOT`는 api·worker.

### 6.7 실패 표시와 자동 재시도

**원인 부류를 도입한다.** Phase 2 `main.ts`의 `scheduleRetry`는 실패 화면이 뜨면 원인과 무관하게 3초·8초
뒤 20초마다 재시도했다(Phase 2 결과 §5.2-1). 실행 게이트가 생긴 뒤 그대로 두면 **마이그레이션 실패가
20초마다 재실행되고, 매번 백업이 하나씩 쌓인다.** 원인 카탈로그(`causes.ts`)의 각 항목에 부류를 붙이고
자동 재시도는 `transient`에만 건다.

| 원인 | 화면이 말하는 것 | 부류 |
| --- | --- | --- |
| 페어링 거부 (§6.2) | 두 경로, "폴더를 옮기거나 지우지 않았는지 확인하세요" | `needs-action` |
| `PG_VERSION` 불일치 | 데이터 폴더의 버전과 앱의 버전 | `needs-action` |
| `pg_controldata` 실패 | 클러스터를 읽을 수 없음, PGDATA 경로 | `needs-action` |
| 소켓 경로 초과 | 경로와 바이트 수 | `needs-action` |
| 고아·락 확인 불가 | pid, 락 파일 경로 | `needs-action` |
| 고아가 내려가지 않음 | pid | `needs-action` |
| `initdb`·`createdb` 실패 | 원인 블록, 로그 경로 | `needs-action` |
| 마이그레이션 상태 조회 실패 | 원인 블록 | `needs-action` |
| `unknown` 마이그레이션 | "더 새 버전의 앱이 이 데이터를 업데이트했어요" | `needs-action` |
| 백업 실패 | 원인(디스크 등), "마이그레이션은 적용하지 않았어요" | `needs-action` |
| 마이그레이션 실패 | 원인 블록, 백업 경로 | `needs-action` |
| 실행 후에도 미적용 (§6.5-5) | 두 트리 불일치 | `needs-action` |
| PG가 준비 전에 죽음 / 준비 유예 초과 | 원인 블록, `logs/postgres/` 경로 | `transient` |
| ready 뒤 PG 사망 (재시작 중) | "데이터베이스를 다시 시작하는 중" | `transient` |
| 외부 디버그 모드의 미적용 | Phase 2 문구 | `transient` (Phase 2 동작 유지) |

- `needs-action`은 **"다시 시도" 버튼과 메뉴로만** 재시도한다. 화면에 자동 재시도 카운트다운을 띄우지 않는다.
- 부류를 모르는 원인(카탈로그 밖의 원문)은 Phase 2대로 `transient`로 둔다 — 새 부류가 기존 복구 경로를
  조용히 끄지 않게 한다.
...
`pg_dump`·`pg_restore`. 번들에서 없으면 기동 전에 원인 표시(`needs-action`, "`pnpm desktop:build`를 다시
실행하세요").
...
Phase 2 §8을 잇는다. Docker 관련 행은 없어지고 아래가 더해진다. 원인 부류는 §6.7.
...
- 확인 환경: packaged. Docker Desktop **완전 종료**(`docker info`가 실패함을 기록). **기존 packaged userData
  ...
  업그레이드 경로 그 자체다.
- 확인 방법: Finder로 실행. 오디오로 회의를 만들고 처리 완료까지 둔다. 키워드로 설명되지 않는 검색 결과가
  있는 검색어로 검색한다.
- 성공 판정: 담화 화면에 진입한다. `<userData>/data/postgres`가 생기고 `_migrations` = 번들 `.sql` 수(현재 24).
  업로드한 회의가 `done`, 전사가 보이고, 의미 검색 결과가 나온다(embed.log에 같은 시각 `POST /embed 200`).
  `storage/meetings/mtg_37`의 체크섬이 불변이다. `config.json`의 바이트가 불변이다. 옛 키는 기본값이므로
  상태 창에 경고가 **없고** `supervisor.log`에 한 줄 있다.

**P3-C2. 재시작 후 데이터 유지**

- 확인 환경: packaged. P3-C1 직후.
- 확인 방법: ⌘Q로 종료. 다시 실행.
- 성공 판정: 같은 회의가 같은 전사로 보인다. `.damwha-cluster`와 `pg_controldata`의 system identifier가 종료 전과
  같다(재초기화 없음). 종료 시 PG 로그에 `database system is shut down`, 재기동 로그에 recovery가 **없다.**
...
- 확인 방법: 앱 실행. `lsof -nP -iTCP -sTCP:LISTEN`, `stat -f %Lp <userData>/run`. 상태 창의 `psql -h …` 명령을
  터미널에 붙여 `SELECT count(*) FROM _migrations`.
- 성공 판정: 번들 postgres의 TCP 리스너가 **0개**. 5432의 리스너는 Docker뿐이다. `run/`이 `700`. 번들 `psql`
  접속이 성공한다. 앱이 정상 진입한다.

**P3-C5. 종료 후 번들 postgres 0개**

- 확인 환경: packaged. 처리 중인 job이 없는 상태.
- 확인 방법: ⌘Q. 5초 뒤 프로세스 목록과 `run/`을 본다.
- 성공 판정: 번들 postgres 프로세스 0개. 소켓 파일이 없다. PG 로그에 `database system is shut down`.
  `supervisor.log`에 postgres `stopped`가 api 뒤에 적혀 있다(역순).
...
- 정리: 앱을 끄고 번들 `psql`로 `_migrations`의 `900_p3_probe.sql` 행과 `p3_probe` 테이블을 지운 **뒤** 두 파일을
  빼고 다시 빌드한다. 그 앱으로 실행해 정상 진입하면 정리가 끝난 것이다. 순서를 바꾸면 P3-C8과 같은 거부를 만난다.

**P3-C8. `unknown` 마이그레이션 거부**

- 확인 환경: dev(`pnpm desktop:dev`)의 클러스터. 앱을 끈 상태에서 번들 `psql`로 `_migrations`에
  `INSERT … VALUES ('999_from_future.sql')`.
- 확인 방법: 앱 실행.
...
- `initdb` 중단 뒤 임시 디렉터리 정리.
- 종료의 immediate 단계와 leaked 보고.

## 10. 제품 코드 변경 목록

`desktop/` 변경과 루트 스크립트 외에, 기존 코드 변경은 **하나**다.

**`be/src/database/migrate.ts`** — `--status` 플래그.
...
| `--locale=C` 클러스터에서 pg_bigm 한글 검색·정렬이 Docker DB(로캘 실측)와 같은가 | 기술 위험 | 계획 1단계. 같은 시드로 비교 |
...
| 5단계(`createdb`·마커)를 부르는 자리 | 구현 세부 | 계획 (§6.4) |
...
| R3-5 | 마이그레이션 실패가 자동 재시도로 반복되고 백업이 쌓인다 | P3-C7 | §6.7 `needs-action` |
...
| 영구 데이터 경로, 접속 설정과 기존 PostgreSQL 인스턴스와의 충돌 방지 | §6.1·§6.2·§6.3, P3-C4·C9·C10 |
...
- **Phase 5**
  - Docker DB → 내장 클러스터 이전. 대상 스토리지는 `<userData>/storage`(Phase 1·2 앱 업로드)와 `be/storage`(웹 흐름).
    둘 다 **Docker DB와 짝**이므로 DB 행과 함께 옮겨야 하고, 이전 뒤 §6.2의 마커를 새 클러스터 id로 적어야 한다.
...
- `desktop/src/services/postgres.ts`(내장·외부 디버그 두 spec), api 어댑터의 실행 게이트, 원인 부류.
- `check-bundle.mjs`·`package.mjs`·`desktop/package.json`·`desktop/.gitignore` 갱신.
- `be/src/database/migrate.ts`의 `--status`와 그 테스트.
