...
- **2026-09-26 (D2 최종 리뷰)** §6.3 행 상태 문구가 `job`을 무시했다 — `downloading`이 없어도(예: CPU
  ...
  계속 보인다.
- **2026-09-26 (D2 실측, desktop dev · 앱 데이터 디렉터리)** C1~C6 모두 통과(C6은 사용자가 토큰을 지우고 넣었다).
  - D2-C6: 토큰 파일이 없는 앱(Electron 창을 원격 디버깅 포트로 조작)에서 화자 분리 모델 폴더를 잠시 치워
    "안 받음"으로 만든 뒤 "화자 분리 모델 받기" → "허깅페이스 토큰이 필요해요" 창이 먼저 뜨고 요청은 나가지 않았다.
    사용자가 토큰을 넣은 뒤에도 job이 생기지 않았고(자동으로 이어 받지 않음), "받기"를 다시 누르자 받아져 "받음"이 됐다.
...
- **2026-09-26 (처리 설정 화면 점검)** §6.4의 "`yes` + 삭제 불가 → 사용 중이면 '지금 설정에서 쓰고 있어요'"를
  뺐다 — 같은 행에 "사용 중" 배지가 이미 붙어 같은 말을 두 번 했다. 배지로 알 수 없는 "처리 중인 작업이 쓰고
  있어요"만 흐린 사유로 남긴다. 409 문구(`model_in_use_by_settings`)는 그대로다.
- **2026-09-26 (처리 설정 화면 점검)** §6.1의 "지금 설정에서 쓰는 모델" 요약을 모델 카드에서 **처리 방식 섹션**으로
  옮기고 이름을 "이 설정으로 쓰는 모델"로 바꿨다. 저장된 값이 아니라 **고르는 중인 폼 값**으로 전사·요약 줄을 만든다
  (렌즈 추출·기본 모델은 설정이 바꾸지 않으므로 그대로) — 고르는 곳과 결과가 떨어져 있어, 프리셋을 눌러도 저장
  전까지 아래 요약이 안 바뀌었고 "사용자 지정"일 때는 무엇을 쓰는지가 접힌 고급 설정 안에만 있었다. "사용자
  지정"은 네 번째 카드로 두어 선택 상태가 늘 카드 하나로 보인다. 모델 카드는 보관함(받아 둔 모델·합계·받기·삭제)만
  맡는다. 목록에 없는 고른 모델(받은 적 없는 다른 백엔드 전사)은 "안 받음" 줄로 보이고 논리 키로 미리 받을 수 있다.
- **2026-09-26 (처리 설정 화면 점검)** §6.3 문구표를 바꿨다 — 받아 둔 모델은 "받음 · 1.6 GB" 대신 **용량만**
  ("1.6 GB"), 안 받은 모델은 "안 받음 · 약 3.1 GB" 대신 흐린 색의 **"약 3.1 GB"**(크기를 모르면 "안 받음")와 "받기"
  버튼. 상태어는 예외 상태(일부만 받음·받는 중·받기 대기 중·지우는 중·확인 중)에만 남긴다. "받음"이 과거 행위로
  현재 상태를 말해 어색했고, "사용 중" 배지·버튼이 이미 상태를 드러낸다. 요약 줄은 "약 3.1 GB · 처음 쓸 때
  받아요", 기본 줄은 모두 받았으면 합계만("2.4 GB"), 프리셋 카드는 "받을 모델 없음". 버튼 동사는 "받기"
  계열을 유지한다.
