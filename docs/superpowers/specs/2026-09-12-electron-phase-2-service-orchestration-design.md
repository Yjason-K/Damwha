...
| `WORKER_ID` | **파일 키가 아니다.** 앱이 실행마다 `desktop-<uuid>`를 새로 만들어 주입한다. 파일에 적지 않고, 이전 빌드가 적어 둔 값이 있어도 무시한다 (2026-09-13, 최종 리뷰 I-4) | §6.5 |
...
0. **자손 스냅샷.** SIGTERM을 쏘기 전에 `descendantPids()`로 자손 집합을 찍어 둔다. 이 시점이
   그 자손을 볼 수 있는 유일한 시점이다 — supervisor가 먼저 죽으면 `--once` 자식과 그것이 띄운
   `mlx_lm.server`는 **pid 1로 재부모화**되어 그 뒤의 ppid BFS에는 영영 보이지 않는다. 강제
   단계로 넘어가기 직전(1단계 유예가 끝나 supervisor가 아직 살아 있음을 막 확인한 지점)에 한 번
   더 찍어 앞의 집합과 합친다 — `--once` 자식은 job마다 새로 뜨므로 진입 스냅샷 하나로는 그 사이
...
4. 그래도 남으면 `descendantPids()`로 자손 집합을 훑어 SIGKILL. `start_new_session`은 세션만 바꾸고
   ...
   **supervisor가 끝난 뒤에는 그룹에 SIGTERM을 한 번 보낸다** (같은 날, I-1의 후속). uv의 그룹에는 uv와
   ...
   - **유예 초과 대화상자가 떠 있는 동안 죽은 경우에도 보내지 않는다** — 그룹 신호도, 3단계 SIGTERM도.
     대화상자는 시간 상한이 없어 "폴 한 번 안"이라는 위 근거가 성립하지 않는다. 사람이 답한 직후
     `handle.alive()`를 다시 보고, 죽어 있으면 찍어 둔 자손의 생존만 보고한다(최종 재리뷰 I-A).
...
타임아웃하거나 죽는다. 그때 "자손 없음"으로 뭉개면 판정이 **거짓 성공** 쪽으로 기운다. §6.5의
`verifyOwnListener`가 소유를 증명하지 못할 때 "내 것이 아니다"로 닫는 것과 같은 규칙을 여기에도
적용한다: 스냅샷을 한 번이라도 못 찍은 종료는 그 뒤 무엇을 관찰했든 **깨끗하다고 보고하지
않는다.** 그래서 종료 결과에는 `{정리됨: false, 남은 pid: []}` — "깨끗하지 않은데 무엇이 남았는지도
모른다" — 라는 상태가 **정상적으로 존재한다.** 종료 대화상자와 상태 창은 이 상태를 "남은 것 없음"과
구별해 표시해야 한다.

**`남은 pid`는 후보이지 증거가 아니다.** 스냅샷은 유예 시간만큼 낡아서, 그 사이 끝난 pid를 OS가
재사용했다면 그 자리에 있는 것은 남의 프로세스다. 그래서 이 목록은 **읽고 보여주는 데만 쓰고
신호를 보내는 데 쓰지 않는다.** 4단계가 실제로 죽이는 대상은 그 자리에서 다시 걷는 트리다.
