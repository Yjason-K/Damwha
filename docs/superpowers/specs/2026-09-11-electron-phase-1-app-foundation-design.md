...
2. 그 포트를 쓸 수 없다고 판정되면 다음 후보 포트로 재기동한다
...
#### 소유권을 판정하는 두 기구 (2026-09-11 개정)

이 절은 원래 "판정은 자식의 bind 결과 하나로 한다 — 미리 빈 포트를 찾아 두는 방식은 탐색과 자식의 bind 사이에 다른 프로세스가 그 포트를 가져갈 수 있다"고 못박았다. 구현이 그 한 기구만으로는 계약을 지킬 수 없다는 것이 실측으로 드러나(P1-C10 (b2): 자식이 `EADDRINUSE`로 죽기 **전에** 첫 probe가 나가고, 그 200을 준 것이 외부 점유자였다) 기구를 둘로 나눴다. 스펙이 구속력을 가지려면 구현이 실제로 도는 알고리즘을 적어야 하므로 여기에 반영한다.

**(a) 스폰 전 사전 점검** — `main.ts`의 `isPortOccupied(port)`. 후보 포트로 TCP 연결을 시도해 누군가 이미 받고 있으면 자식을 **아예 띄우지 않고** 다음 후보로 넘어간다. 값싸고, 흔한 경우(외부 API가 그 포트를 쥐고 있음)를 스폰조차 없이 막는다.

**(b) 준비 후 소유 증명** — `main.ts`의 `verifyOwnListener(port, childPid)`. `/api/health`가 200을 주더라도 그것만으로는 준비로 인정하지 않는다. `lsof -sTCP:LISTEN -t`로 그 포트에서 **실제로 LISTEN 중인 pid**를 얻고, `ps -axo pid,ppid`를 BFS로 훑어 얻은 자식의 자손 집합과 대조해, 리스너가 우리 자식이거나 그 자손일 때만 `ready`로 판정한다. 아니면 "아직 준비 안 됨"으로 돌려보내 폴링을 계속한다. 판정 도구(`lsof`/`ps`) 자체가 실패하면 소유를 증명할 수 없으므로 안전하게 "아니오"로 본다 — 준비 판정은 실패 쪽으로 닫는다.

**둘을 다 두는 이유.** (a)만 두면 원래 스펙이 경고한 그대로다 — 점검과 자식의 bind 사이에 다른 프로세스가 그 포트를 가져가는 TOCTOU 창이 열린다. **그 창을 닫는 것이 정확히 (b)다.** (a)가 놓친 점유자는 (b)의 pid 대조에서 반드시 걸린다. 우리 자식이 뒤이어 `EADDRINUSE`로 죽으면 기존 stderr 분기가 다음 포트로 넘긴다. 반대로 (b)만 두면 흔한 경우마다 자식을 띄웠다 죽이는 낭비와 30초 타임아웃을 치른다. 그러므로 (a)는 성능, (b)는 정확성을 맡고, **계약("포트 응답을 준비 신호로 쓰지 않는다")을 실제로 지키는 것은 (b) 하나다.**

자손까지 보는 이유는 자식의 모양이 두 가지이기 때문이다 — dev의 자식은 `pnpm` → `nest`(CLI) → `node dist/main` 체인이라 실제로 bind하는 것은 추적 중인 pid의 **손자**이고, packaged의 자식은 `utilityProcess` 헬퍼 자신이다. 자손 집합에 자기 자신을 포함시켜 양쪽을 한 판정으로 덮는다.

#### 이 설계가 바꾼 관찰 가능한 동작

`pnpm dev`가 3000을 쓰고 있어도 앱이 뜬다 — 로드맵의 "기존 웹 개발·실행 흐름과 충돌하지 않음"을 포트 충돌 없이 지난다. **단, 그 방식이 바뀌었다.**

- **이전:** `pnpm dev`의 API는 `HOST` 기본값 `0.0.0.0` 때문에 `*:3000`에 bind하고, 앱의 자식은 `SO_REUSEADDR` 아래 `127.0.0.1:3000`을 그 위에 **겹쳐** bind했다. loopback 트래픽은 더 구체적인 bind가 가져가므로 둘 다 정상 동작했다. 폴백은 일어나지 않았다.
- **이후:** 기구 (a)가 `127.0.0.1:3000`에 연결해 보고 `*:3000`의 리스너가 그 연결을 받으므로, 앱은 3000을 점유된 것으로 보고 **항상 다른 포트로 옮긴다.** 겹쳐 bind하던 동작은 사라졌다.

바뀐 쪽이 옳다. 겹쳐 bind는 "누가 그 포트를 쥐고 있는가"를 앱이 모른 채 우연히 성립한 것이고, 같은 상황에서 외부 리스너가 `127.0.0.1`에 bind해 있기만 하면 곧장 P1-C10 (b2)의 오인으로 넘어간다. 대가는 dev가 도는 동안 앱이 고정 포트를 쓰지 못한다는 것뿐이며, §6.4가 이미 "포트가 바뀌어도 다른 서비스에 영향이 없다"고 닫아 둔 범위 안이다.
