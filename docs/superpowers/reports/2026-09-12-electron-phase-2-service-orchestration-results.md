...
7. **종료·닫기 경합 유도 조건은 실행하지 않았다.**
   - 배경: Task 13 재리뷰가 Task 15에 지정했다. `main.ts`의 종료 흐름은 vitest가 부를 수 없고 실패가 조용한 곳이
     많아, 일부러 만들지 않으면 packaged 점검도 통과해 버린다는 이유였다.
   - 이번 검증이 밟은 것은 정상 경로뿐이다: 분석 중·녹음 중·렌즈 중 ⌘Q 승인, 창 닫기 뒤 Dock 복귀.
   - 아래는 **아무도 해 보지 않았다.**
     - 멎은 렌더러에서 ⌘Q·⌘W
     - handshake 도중이나 서비스 정지 도중 두 번째 ⌘Q·⌘W
     - 확인 대화상자를 취소한 뒤 다시 ⌘W
     - 창을 닫아 창이 없는 상태에서 ⌘Q
     - ⌘W 확인이 떠 있는 동안 ⌘Q(와 반대)
     - 종료 중 Dock 클릭(새 창이 생기면 안 된다)
     - 종료 중 앱 두 번째 실행
   - 흐름 래치(`createFlowLatch`)의 모든 교차는 최종 리뷰가 코드로 추적해 옳다고 판정했지만, 실앱 확인은 아니다.
   - `pnpm desktop:dev`에서 종료 뒤 Vite 자식이 남지 않는지도 이번에 보지 않았다(packaged에는 Vite가 없다).
