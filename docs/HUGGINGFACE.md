...
Damwha 데스크톱 앱은 토큰 없이도 켜진다 — 회의를 새로 기록하거나 재처리할 때만 토큰이 필요하고,
회의 목록을 보거나 검색하는 데는 아무것도 필요 없다.

처음 실행하면 담화 화면 안에 "담화를 쓰려면 허깅페이스 토큰이 필요해요" 대화상자가 뜬다. 모델
사용 조건 페이지와 토큰 발급 페이지로 바로 이동하는 버튼이 같이 있다. **나중에 하기**를 누르면 이번 실행에서는
...
토큰을 저장하면 앱이 그 자리에서 허깅페이스에 토큰이 유효한지 묻는다(계정 확인). 실패하면
입력칸 바로 아래에 이유가 뜬다. 라이선스 수락 여부는 이때 확인하지 않는다 — 수락하지 않았으면
첫 회의를 처리할 때 그 회의에 원인과 "사용 조건 페이지 열기" 버튼이 뜬다:

| 나오는 말 | 원인 |
|---|---|
| 저장 시: 토큰이 유효하지 않아요 (401) | 토큰이 잘못됐거나 복사 중 잘렸다 → 3번 |
| 회의 처리 시: 모델 사용 조건에 동의하지 않았어요 (403) | 그 저장소를 안 눌렀다 → 2번. 라이선스를 다 눌렀는데도 나오면 수락한 계정과 토큰 발급 계정이 다르거나, Fine-grained 토큰에 gated repo 권한이 없다 |

검증을 통과하면 앱이 바로 회의 처리를 시작할 수 있다. 모델 가중치는 첫 회의를 처리할 때 알아서
받는다(수 GB, 수 분). 첫 회의 전에 **설정 › 모델**에서 미리 받아 둘 수도 있다([모델](MODELS.md)).
