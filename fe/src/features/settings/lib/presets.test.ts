...
describe("PRESET_META — 요약 모델", () => {
  it("프리셋별 요약 모델 매핑이 BE와 일치한다", () => {
    ...
    expect(PRESET_META.standard.summary_model).toBe("qwen3.5:9b-mlx");
    expect(PRESET_META.quality.summary_model).toBe("qwen3.5:27b-mlx");
  });
  ...
  it("BE 프리셋 revision과 맞춘다", () => {
    expect(PRESET_META_REVISION).toBe("2026-08-12.2");
  });
});
