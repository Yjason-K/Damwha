...
- **전사 언어는 자유 텍스트가 아니라 카탈로그 셀렉트다** (`STT_LANGUAGE_OPTIONS`, `sttLanguageOptions`, `isSttLanguage`). Whisper는 언어 토큰을 **하나만** 받으므로 다중 선택은 애초에 보낼 곳이 없다 — 한국어 문장에 섞인 영어 단어는 `ko` 하나로 그대로 전사된다. 셀렉트 아래 안내문이 그 사실을 말한다. `auto`는 "언어를 지정하지 않음"이고 워커가 whisper 자동 감지로 옮긴다(파일당 한 언어로 확정). 서버는 **쓰기만** 조이므로(`ProcessingSettingsUpdate.language`가 `SttLanguage`, `ProcessingConfig.language`는 `string`) 카탈로그 밖 값이 내려올 수 있다 — `sttLanguageOptions`가 그 값을 목록 끝에 얹고, 저장 버튼은 막힌 채 이유를 버튼 위에 적는다. 안내를 고급 설정 안에 두면 접혀 있을 때 비활성 버튼의 이유가 안 보인다.
...
`src/features/models/` (모델) owns the Settings › 모델 card: `api` (`useModels` — `GET /models`, polls every 3 s only while the server says `pending`), `lib` (`rows.ts` — row copy, default visibility, the "이 설정으로 쓰는 모델" summary lines — `summaryLines(view, pick)` builds the 전사·요약 lines from the **form's unsaved pick**, 렌즈 추출·기본 from the server; `format.ts` — 1000-based sizes), `ui` (`ModelsCard` = the storage list only; `ModelsInUse` = the summary, rendered inside the 처리 방식 section of `ProcessingSettingsForm`). Short model names come from `modelShortLabel` in `features/settings/lib/presets.ts` — do not copy the label Records. Saving processing settings invalidates `["models"]`. D2: `lib/actions.ts`가 행 버튼(`rowAction`)과 job 오류 문구(`jobErrorText`, **code로만** 고름)·409 문구(`conflictText`)를 정한다. `ApiError.code`는 서버 본문의 `code`를 항상 싣는다. 화자 분리 모델 받기는 `useDiarizationGate().run()`을 거친다.
...
**The mark lives in four files, not two.** `shared/ui/brand-mark.tsx` and `public/favicon.svg` hold the geometry; `public/favicon.ico` (16/32/48, PNG-in-ICO) and `public/apple-touch-icon.png` (180×180) are rasters derived from that same geometry, and `index.html`'s `theme-color` is a fifth place the base hue appears. The sixth is outside `fe/`: the desktop app icon `desktop/build-resources/icon.svg` (same geometry on the macOS 824-in-1024 grid) the `icon.icns` that `desktop/scripts/build-icon.sh` renders from it via headless Chrome, and an inline copy on the startup screen `desktop/shell/status.html`. Change one, change all six — the rasters had already drifted once. There is no SVG rasteriser on this machine; they were regenerated by drawing the SVG onto a `<canvas>` in a browser and reading `toDataURL()`, which preserves the transparent corners that a page screenshot would flatten to white. The fills are **flat** — no gradients — because the mark has to hold up at 16px.
