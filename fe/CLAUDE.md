...
- The **shadcn token contract** (`--background`, `--primary`, `--sidebar-*`, …) is mapped _onto_ those Timbre semantics at the bottom of the `:root` block, so shadcn components render on-brand automatically. Caveat: shadcn's `--accent` means "hover/subtle surface", and the signal orange is `--primary` (not `--accent`).
- `@theme inline` / `@theme` blocks expose these as Tailwind utilities (`bg-primary`, `text-spk-1-text`, `rounded-md`, dense `text-*` scale, Sofia Sans / DM Mono fonts).

**The palette is retoned after the Mastercard reference** (`DESIGN-mastercard.md` at the repo root): warm putty-cream neutrals, warm ink text, a single signal-orange accent ramp, and a radius ladder raised one notch. The retone moved **values under the semantic alias layer only** — `--surface-*` / `--text-*` / `--border-*` and the shadcn contract kept their structure, so no component had to change. Boundary, and it is a hard one: **density (the 14px-base `text-*` scale, `--rail-*` widths, spacing), layout devices, and the brand mark were not part of the retone.** The reference's circular image masks, orbital arcs, satellite CTAs and watermark headlines are structure, not color, and were deliberately refused — the three-pane shell has nowhere to put them. Its extreme radii were refused too, except `--radius-full` for genuinely pill-shaped things (avatars, dots, toggles); a 260–420px rail cannot carry 20px+ corners on its list rows without them fusing together.
