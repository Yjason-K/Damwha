...
**The mark lives in four files, not two.** `shared/ui/brand-mark.tsx` and `public/favicon.svg` hold the geometry; `public/favicon.ico` (16/32/48, PNG-in-ICO) and `public/apple-touch-icon.png` (180×180) are rasters derived from that same geometry, and `index.html`'s `theme-color` is a fifth place the base hue appears. The sixth is outside `fe/`: the desktop app icon `desktop/build-resources/icon.svg` (same geometry on the macOS 824-in-1024 grid) and the `icon.icns` that `desktop/scripts/build-icon.sh` renders from it via headless Chrome. Change one, change all six — the rasters had already drifted once. There is no SVG rasteriser on this machine; they were regenerated by drawing the SVG onto a `<canvas>` in a browser and reading `toDataURL()`, which preserves the transparent corners that a page screenshot would flatten to white. The fills are **flat** — no gradients — because the mark has to hold up at 16px.
