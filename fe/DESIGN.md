...
참조 계열은 **Cursor** · Linear · GitHub다. Notion · Airbnb 같은 소비자 앱
미학이 아니다. 장식은 정보를 밀어내지 않는 선까지만 허용된다.

Cursor에서 가져온 것은 **톤뿐이다** — 따뜻한 크림 바탕, 따뜻한 먹색 잉크,
아껴 쓰는 오렌지 한 색, hairline 위계, 한 단계 완화한 radius 스케일,
Inter + JetBrains Mono. Cursor의 **editorial 리듬(큰 본문 · 넓은 섹션
간격)은 가져오지 않았다.** 그건 세로로 긴 마케팅 페이지의 리듬이고,
여기는 3분할 읽기 셸이다 — 위의 우선순위 3(정보 밀도)이 이긴다.
`--text-*` 스케일과 `--rail-*` 폭은 Cursor 리톤과 무관하게 유지된다.

## 2. 시각 방향

- **라이트 전용.** 다크 모드는 구현되어 있지 않다 (9장 참조)
- **바탕은 흰색이 아니라 따뜻한 크림이다**(`--surface-app`). 잉크도 순검정이
  아니라 따뜻한 먹색(`--text-primary`)이다. 중립 스케일(`--gray-*`) 전체가
  warm 계열이므로, 차가운 회색을 새로 들여오면 화면에서 튄다
- **위계는 border로 만든다.** 그림자는 실제로 떠 있는 레이어에만
- **강조색은 하나.** 아껴 쓰는 오렌지(`--accent-solid`) 하나로 상호작용
  가능한 것과 선택된 것을 표시한다. **아껴 쓴다는 게 규칙의 일부다** —
  새 표면에 solid 오렌지를 얹기 전에 그 화면에 이미 몇 개가 떠 있는지
  세어 본다. 옅은 강조(`--accent-bg`)로 충분한 자리에 solid를 쓰지 않는다
...
> 브랜드색이 아니다. 브랜드 오렌지는 `--primary`다. `bg-accent`를 강조색
> 버튼에 쓰면 회색 버튼이 나온다.
...
Inter / JetBrains Mono. Inter는 Cursor의 라이선스 서체 CursorGothic의
공개 대체재이고, JetBrains Mono는 코드·타임스탬프·단축키 같은 모노 표면
전부를 맡는다. **브라우저 기본값보다 한 단계 조밀한 스케일**이다
...
**Cursor를 닮게 하겠다고 본문을 키우지 말 것.** 크기 스케일은 리톤 대상이
아니었다 (1장 참조).
...
- **차가운 회색·파랑을 새로 들여오지 말 것.** 중립은 warm(`--gray-*`),
  강조는 오렌지(`--accent-*`) 하나다. 인디고 계열은 브랜드 마크 안에만 남아
  있고 (2장의 예외), UI 표면으로 새어나오면 안 된다
