...
참조 계열은 Mintlify의 **문서 표면**이다 — 무채색 잉크 위에 민트 한 점,
평면적인 면과 hairline 경계선, Inter + 모노 짝. 같은 Mintlify라도 **마케팅
페이지 쪽은 참조 대상이 아니다.** 그쪽의 그라디언트 히어로·넓은 본문·큰
섹션 간격을 가져오면 위 우선순위 3(정보 밀도)이 먼저 무너진다. Notion ·
Airbnb 같은 소비자 앱 미학도 아니다. 장식은 정보를 밀어내지 않는 선까지만
허용된다.
...
- **강조는 두 역할로 나뉘고, 그 분리가 의도다.** 주 버튼의 면은 무채색
  ...
  (`--accent-bg` / `--accent-text` / `--focus-ring`)다. 잉크 하나로 통일하면
  선택된 행(`--accent-bg`)과 hover된 행(`--surface-hover`)이 둘 다 회색이
  되어 구분이 사라진다. 신호색을 늘리라는 뜻이 아니라, **한 강조색이
  맡던 두 역할을 면과 신호로 갈랐다**는 뜻이다
- **화자 팔레트가 유일한 다색 요소다.** 8개 색상(`--spk-1..8`)은 화자
  식별에만 쓴다. 다른 용도로 끌어다 쓰면 "이 색이 화자를 뜻한다"는
  화면 전체의 약속이 깨진다
- **평면적.** 그라디언트, 글래스모피즘, 큰 둥근 카드, 카드 안의 카드 없음
- **브랜드 마크는 예외다.** 앱 아이콘만 고정 색을 쓴다. 채움은 **평면**이다 —
  파비콘 크기까지 줄어들어도 살아남아야 해서 그라디언트를 쓰지 않는다. 마크는
  어디에 놓이든 같게 읽혀야 해서 표면/텍스트 시맨틱을 따르지 않는다 — 토큰을
  바꿨다고 아이콘이 따라 바뀌면 안 된다. 색은 강조색과 **같은 계열**로
  맞추되(안 그러면 좌상단에서 혼자 튄다) 값은 토큰을 참조하지 않고 직접 적는다.
  이 예외는 마크 안에서 끝나고, UI 표면으로 새어나가지 않는다
- **마크는 다섯 군데에 흩어져 있다.** `BrandMark`와 `public/favicon.svg`가
  도형을, `public/favicon.ico`와 `public/apple-touch-icon.png`가 거기서 뽑은
  래스터를, `index.html`의 `theme-color`가 바탕 색조를 들고 있다. **하나를
  고치면 다섯 다 고친다** — 실제로 래스터 두 개가 한 번 뒤처진 적이 있다. **톤을 통째로 갈아엎을
  때도 마크는 대상이 아니다** — 2026-08 리톤은 `index.html`의 `theme-color`
  까지 포함해 마크를 손대지 않았다

> **리톤의 경계.** 2026-08에 이 문서의 참조 계열이 바뀌면서 움직인 것은
> **색·서체·radius뿐**이다. 타입 스케일(4장), 레일 폭과 3분할 배치(5장),
> 컴포넌트 구조와 인터랙션 상태(6·7장), 브랜드 마크는 **리톤 대상이
> 아니었다.** 참조 계열을 더 닮게 만들겠다고 본문을 키우거나 섹션 간격을
> 벌리지 마라 — 그건 리톤이 아니라 1장의 우선순위를 뒤집는 일이다.
...
> `bg-accent`를 주 버튼에 쓰면 회색 버튼이 나온다. 반대로 **선택·링크
> 신호를 `--accent-solid`로 칠하지도 마라** — 그건 잉크색이라 hover 회색과
> 구분이 안 된다. 신호는 `--accent-bg` / `--accent-text` 쪽이다.
...
Inter(본문·UI) / Geist Mono(코드·수치). 서체는 이 두 벌로 끝이다 — 세 번째
서체를 들이지 마라. 이탤릭도 쓰지 않는다. 강조는 굵기·색·면으로 만든다.

**브라우저 기본값보다 한 단계 조밀한 스케일**이다(`--text-base`가 기준점).
웹 기본 크기를 가정하고 눈대중으로 고르지 말고, 아래 표에서 용도로 고를 것.

> 참조 계열의 본문 크기는 담화보다 한 단계 크다. **그건 의도적으로 따라가지
> 않았다** — 그쪽은 한 페이지짜리 문서고 담화는 3분할 셸이라, 같은 크기를
> 쓰면 한 화면에 들어가는 발화 수가 줄어든다. 이 스케일은 리톤 대상이
> 아니었으니 "톤을 맞춘다"는 이유로 키우지 마라.
...
- **새 색·radius·간격 값을 임의로 만들지 말 것.** 기존 토큰으로 안 되면
  ...
  일회용 값을 심지 않는다.
- **참조 계열을 닮게 만들겠다고 밀도를 풀지 말 것.** 본문 크기, 레일 폭,
  섹션 간격은 리톤 대상이 **아니었다**(2·4장). 참조 계열의 마케팅 페이지
  리듬 — 그라디언트 히어로, 넓은 본문, 큰 섹션 간격, 풀 라운드 알약
  버튼 — 은 문서 표면이 아니라 랜딩용이라 여기 들어오면 1장의 우선순위
  3이 무너진다.
- **브랜드 마크를 톤에 맞춰 다시 칠하지 말 것.** 마크는 시맨틱 밖의
  예외이고(2장), 리톤 대상이 아니었다.
- **`--accent-solid`를 선택 상태에 쓰지 말 것.** 그건 주 버튼 면용
  잉크색이다. 선택은 `--accent-bg` 면 + `--accent-text` 글자다(7장).
- 그라디언트, 글래스모피즘, 큰 둥근 카드, 카드 중첩 없음
- 장식용 아이콘을 넣지 않는다. 아이콘은 뜻이 있을 때만
- 화자 팔레트를 화자 아닌 것에 쓰지 않는다. 반대로 **`--spk-*`를 새로
  고를 때는 `-solid` 위에 흰 이니셜이 얹힌다는 것**(`avatar.tsx`)과
  8색이 색상환에서 충분히 벌어져야 한다는 것을 함께 확인한다
