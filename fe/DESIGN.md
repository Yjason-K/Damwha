...
참조 계열은 **IBM Carbon**이다. Notion · Airbnb 같은 소비자 앱 미학이
아니다. 장식은 정보를 밀어내지 않는 선까지만 허용된다.

Carbon을 고른 이유는 그것이 원래 밀도 높은 업무용 UI를 위한 시스템이기
때문이다. 다만 **가져온 것은 톤(색·서체·모서리)뿐이고, IBM 마케팅
페이지의 넓은 여백과 큰 디스플레이 타입은 가져오지 않았다.** 담화의
간격·타입 스케일·레일 폭은 리톤 대상이 아니었고 그대로다.

## 2. 시각 방향

- **라이트 전용.** 다크 모드는 구현되어 있지 않다 (9장 참조)
- **위계는 border로 만든다.** 그림자는 실제로 떠 있는 레이어에만
- **강조색은 하나.** IBM 블루(`--accent-solid`) 하나로 상호작용 가능한
  것과 선택된 것을 표시한다. 두 번째 브랜드색을 만들지 않는다
- **모서리는 각지다.** Carbon의 서명이다. `--radius-*`는 통째로 아주
  낮은 대역으로 내려와 있어서 스케일 안에서 값이 겹치기도 한다 — 범위
  자체가 좁아진 것이 의도다. `--radius-full`만 예외로 원형을 유지하며,
  아바타와 pill이 그 위에 선다
...
- **브랜드 마크는 예외다.** 앱 아이콘(`BrandMark`, `public/favicon.svg`)만
  ...
  안에서 끝나고, UI 표면으로 새어나가지 않는다. **Carbon 리톤 때도 마크는
  건드리지 않았다** — 마크는 시맨틱 밖의 상수다

화자 팔레트는 Carbon의 다색 램프에서 한 계열씩 가져온다. `-solid`는 흰
이니셜을 받는 면이라 흰 글자 대비를 지켜야 하고, `-bg`/`-text`는 짝으로
쓰인다. **블루 계열은 화자에 쓰지 않는다** — 강조색이 이미 그 자리를
차지하고 있어서, 파란 화자는 "선택된 항목"으로 잘못 읽힌다.
...
> 강조색이 아니다. IBM 블루는 `--primary`다. `bg-accent`를 강조색
> 버튼에 쓰면 회색 버튼이 나온다.
...
IBM Plex Sans / IBM Plex Mono. **브라우저 기본값보다 한 단계 조밀한
스케일**이다 (`--text-base`가 기준점). 웹 기본 크기를 가정하고 눈대중으로
...
**Carbon의 display용 light 굵기는 도입하지 않았다.** Plex Sans의 300은
큰 히어로 타입에서 성립하는 브랜드 시그니처인데, 담화의 제목은 스케일
전체가 한 단계 작은 dense 3분할 셸 안에 있다. 그 크기에서 300으로 내리면
제목이 패널 배경에 묻혀 위계가 무너진다. 서체 패밀리만 바꾸고 `--weight-*`
토큰은 그대로 뒀다.
...
- **raw hex나 임의 색값을 쓰지 말 것.** `bg-[#4F46E5]` ✗.
  ...
  아직 없어서 생긴 구멍이다. 리톤 때 값만 다시 맞췄고 **개수는 3곳
  그대로**다. 늘리지 말 것.)
...
- **새 색·radius·간격 값을 임의로 만들지 말 것.** 기존 토큰으로 안 되면
  ...
  일회용 값을 심지 않는다.
- **모서리를 다시 둥글리지 말 것.** 각진 모서리가 지금의 톤이다.
  `--radius-*` 밖의 임의 곡률을 컴포넌트에 심지 않는다. 원형이 필요한
  곳은 `--radius-full`뿐이다.
- **밀도를 Carbon 마케팅 쪽으로 늘리지 말 것.** 리톤은 색·서체·모서리에
  한정됐다. 타입 스케일·레일 폭·간격은 담화 것이고, 참조 계열이 바뀌었다는
  이유로 여백을 키우지 않는다.
