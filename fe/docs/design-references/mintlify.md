...
> **이 문서는 담화의 디자인 문서가 아니라 외부 참조 스펙이다.**
>
> 2026-08 리톤(PR #6)이 참조한 Mintlify 톤 분석본이고, 그때 받은 그대로 둔다 —
> 날짜 스냅샷이라 사후에 고치지 않는다. 담화가 실제로 쓰는 값은
> `fe/src/index.css`가 유일한 SoT이고, 디자인 의도는 `fe/DESIGN.md`에 있다.
>
> **여기 적힌 값을 그대로 가져다 쓰지 말 것.** 리톤은 옮겨 적기가 아니었다.
> 이 스펙의 26색 중 담화가 그대로 받은 건 13색(중립·잉크 계열)뿐이고,
> `index.css`의 나머지 39색은 담화 고유 요구에 맞춰 새로 만든 파생값이다.
>
> - 이 스펙의 brand-green은 흰 바탕 대비가 링크색 기준에 한참 못 미쳐,
>   같은 색상의 어두운 단계를 새로 잡아 `--accent-11`로 썼다
> - **화자 8색 팔레트는 이 스펙에 개념 자체가 없다.** 아바타 흰 이니셜
>   대비 요구에 맞춰 24개 값을 전부 새로 만들었다
> - 스펙의 error red도 흰 글자 대비가 모자라 한 단계 어둡게 했다
>
> 안 가져온 것도 의도적이다 — 마케팅 히어로 그라디언트, 다크 표면,
> pill 버튼(모든 버튼 full radius), 타이포 스케일, 섹션 리듬. 담화는
> 3분할 dense 읽기 셸이라 밀도가 이긴다. `fe/DESIGN.md` 1장 참조.
