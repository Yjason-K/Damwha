...
- `pages/meeting.tsx`: `onSeek={seek}` 배선 그대로 (놓을 때 fraction 1회 수신).
...
새 컴포넌트 테스트 `src/shared/ui/speaker-timeline.test.tsx`
(오버레이 레인 셀의 `getBoundingClientRect`를 목킹해 fraction 계산 고정):
