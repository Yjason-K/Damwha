...
describe('resolvePreset — 요약 모델', () => {
  it('프리셋별 요약 모델 매핑', () => {
    ...
    expect(resolvePreset('standard', 'ko').summary_model).toBe('qwen3.5:9b-mlx');
    expect(resolvePreset('quality', 'ko').summary_model).toBe('qwen3.5:27b-mlx');
  });
  ...
  it('프리셋 정의가 바뀌었으므로 revision을 올린다', () => {
    expect(PRESET_REVISION).toBe('2026-08-12.2');
  });
});
