...
- **Python ML worker (`worker/`, Python)** — a supervisor process polls `job` and spawns a one-job child that runs ffmpeg normalize+probe → VAD → diarization → speaker ID → STT → align → overlap re-STT, writing `utterance`/`meeting_cluster`/`voiceprint` rows. No HTTP. **Implemented** (Plan 2); see the worker section below.
