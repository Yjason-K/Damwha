...
The system is a **polyglot** split that communicates **only through Postgres** — never HTTP between them. The `job` table is the whole contract in both directions; the one other shared row is `app_setting.worker_capabilities`, which the worker writes and the API only reads (see Processing settings below):
