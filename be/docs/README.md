...
- **인계 (`handoff/`)** — 작업을 중단한 시점의 재개용 메모. 다른 기기·다른 날에 이어 붙이기 위한 것이라 **저장소 밖에 있는 것**(기기 로컬 오디오, 모델 캐시, 커밋하지 않은 진단 스크립트)과 재시작 명령을 담는다. 수명이 짧다 — 지속되는 미해결 항목은 반드시 [backlog.md](./backlog.md)에도 등록해서, 인계 문서가 낡아도 남게 한다.
  - [handoff/2026-08-13-stt-qwen.md](./handoff/2026-08-13-stt-qwen.md) — STT 반복 루프 수정 직후, Qwen3-ASR 판단 보류 시점.
