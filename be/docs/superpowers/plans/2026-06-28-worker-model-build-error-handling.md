...
- **검증 게이트**: `cd worker && uv run pytest -q` 전체 통과 + `uv run ruff check . && uv run ruff format --check .` (Docker 필요: testcontainers). 커밋 전에는 항상 `uv run ruff format .`(적용)을 먼저 실행해 line-length 100 줄바꿈을 자동 정리한다 — `--check`는 파일을 고치지 않으므로 게이트만 실패시킨다.
...
- `worker/damwha_worker/__main__.py` — `handle_job`/`run_once` 빌더 콜백화, `dispatch_claimed_job` 헬퍼 + `main()` 재작성 + registry 호이스팅 (Task 2, **단일 원자 커밋** — 시그니처 변경과 `main()` 호출부 갱신이 한 커밋에 들어가야 워커가 깨지지 않음)
- `worker/tests/test_worker_loop.py` — 호출부 갱신 + 빌드 실패/배선 테스트 (Task 2)
...
- [ ] **Step 5: Lint (format apply → check → verify) + commit**

```bash
cd worker && uv run ruff format . && uv run ruff check . && uv run ruff format --check .
...
### Task 2: 빌드를 guarded 콜백으로 + 테스트 가능한 dispatch + main 재작성 (단일 원자 커밋)

**Files:**
- Modify: `worker/damwha_worker/__main__.py` (imports, `handle_job`, `run_once`, 신규 `dispatch_claimed_job`, `main()` 재작성)
- Modify: `worker/tests/test_worker_loop.py` (신규 빌드/배선 테스트 + 기존 호출부 갱신)
- Modify: `worker/tests/test_dispatch_index.py` (호출부 갱신)
- Modify: `worker/scripts/smoke_process_meeting.py:109` (호출부 갱신, CI 아님)

**Interfaces:**
- Consumes: Task 1의 `classify`(import류 → PERMANENT), `db.claim`/`db.requeue`/`db.fail_*`, `parse_payload`, `run_process_meeting`/`run_enroll_speaker`/`run_index_meeting`
- Produces (테스트가 의존하는 정확한 시그니처):
  ...
  - `build_models`/`build_text_embedder`는 **무인자 콜백**(`build_models() -> Models`, `build_text_embedder() -> TextEmbedder`), `handle_job`의 `try` 안에서 타입에 맞는 콜백만 호출. `build_models_fn(payload, settings)`/`build_text_embedder_fn(settings)`는 registry 함수, `heartbeat_cm`은 컨텍스트매니저.

> **왜 원자 커밋인가:** `handle_job` 시그니처(`models=`/`text_embedder=` → 콜백)를 바꾸면, 현재 `main()`이 `handle_job(..., models=...)`/`(..., text_embedder=...)`를 호출하던 부분이 `TypeError`로 깨진다(테스트는 `main()`을 안 돌려 통과해도 실 워커는 크래시). 따라서 시그니처 변경·`dispatch_claimed_job` 도입·`main()` 재작성·전 호출부 갱신·전 신규 테스트를 **한 커밋**에 넣어 워커 실행 가능성과 스위트 green을 동시에 보장한다.

- [ ] **Step 1: Write handle_job-level build-failure tests (red)**

`worker/tests/test_worker_loop.py` 상단 import를 다음과 같이 보강한다:
- `from tests.conftest import seed_job, seed_meeting` → `from tests.conftest import seed_job, seed_meeting, seed_speaker`
- (기존 `from damwha_worker.errors import ErrorKind, WorkerError` 유지)
...
def test_enroll_build_failure_fails_speaker(conn, tmp_path):
    ...
    assert (
        conn.execute(
            "SELECT enrollment_status FROM speaker WHERE id=%s", (sid,)
        ).fetchone()["enrollment_status"]
        == "failed"
    )
```

- [ ] **Step 2: Write dispatch_claimed_job wiring tests — process + index (red)**

같은 파일 `test_worker_loop.py` 상단 import에 추가:
- `from types import SimpleNamespace`
- `from damwha_worker.__main__ import handle_job, run_once` → `from damwha_worker.__main__ import dispatch_claimed_job, handle_job, run_once`

파일 끝에 spy CM + 2개 배선 테스트 추가. process 경로는 `build_models_fn`이, index 경로는 `build_text_embedder_fn`이 **heartbeat 내부에서 정확히 1회** 호출되고 상대 빌더는 호출되지 않음을 검증한다:
...
Run: `cd worker && uv run pytest tests/test_worker_loop.py -q -k "build_failure or build_transient or dispatch_"`
Expected: FAIL — `TypeError: handle_job() got an unexpected keyword argument 'build_models'` 및 `ImportError: cannot import name 'dispatch_claimed_job'`.

- [ ] **Step 4: Refactor imports + `handle_job` (full function)**

`worker/damwha_worker/__main__.py` 상단 import에서 `Models`를 제거(시그니처에서 더는 타입힌트로 안 씀 → ruff `F401` 방지):

```python
from .pipeline.process_meeting import run_process_meeting
```

`handle_job`을 아래 **완전한 함수**로 교체(변경점은 시그니처와 `try` 본문의 빌더 호출뿐, `except` 블록은 기존과 동일하게 전체 포함):
...
- [ ] **Step 6: Add `dispatch_claimed_job`**
...
- [ ] **Step 7: Rewrite `main()` (registry/Heartbeat import를 claim 이전으로 호이스팅)**
...
- [ ] **Step 8: Migrate all existing call sites (한 커밋 내 호환)**

`tests/test_worker_loop.py`:
- `test_run_once_processes_to_done`: `run_once(conn, "w1", _models(), Storage(str(tmp_path)))` → `run_once(conn, "w1", Storage(str(tmp_path)), build_models=_models)`.
...
- `test_permanent_error_fails`: `models=boom` → `build_models=lambda: boom`.

`tests/test_dispatch_index.py`:
- `test_index_permanent_failure_fails_job_only`: `text_embedder=RaisingTextEmbedder(ErrorKind.PERMANENT)` → `build_text_embedder=lambda: RaisingTextEmbedder(ErrorKind.PERMANENT)`.
- `test_index_transient_failure_requeues`: `text_embedder=RaisingTextEmbedder(ErrorKind.TRANSIENT)` → `build_text_embedder=lambda: RaisingTextEmbedder(ErrorKind.TRANSIENT)`.
- `test_run_once_handles_index_job`: `run_once(conn, "w1", None, Storage(str(tmp_path)), text_embedder=FakeTextEmbedder())` → `run_once(conn, "w1", Storage(str(tmp_path)), build_text_embedder=lambda: FakeTextEmbedder())`.

`scripts/smoke_process_meeting.py:109` (CI 아님): `outcome = run_once(conn, "smoke-worker", models, storage)` → `outcome = run_once(conn, "smoke-worker", storage, build_models=lambda: models)` (line 103의 `models = build_models(payload, settings)` eager 빌드는 유지).

- [ ] **Step 9: Run the full worker suite**

Run: `cd worker && uv run pytest -q`
Expected: PASS — 신규 6종(build-failure 4 + dispatch 2) 포함 전체 통과, 마이그레이션된 기존 테스트도 green.

- [ ] **Step 10: Lint (format apply → check → verify) + commit**
...
git commit -m "feat(worker): build models via guarded callback + testable dispatch_claimed_job (no crash on build failure)"
```
...
```bash
cd worker && uv run ruff format --check . && uv run ruff check . && uv run pytest -q
```
Expected: ruff 클린, 전체 테스트 PASS.

- [ ] **워커 모듈 import 정합성** — `main()`은 `pragma: no cover`라 직접 실행하지 않지만, 시그니처/호출부 정합성은 모듈 import로 빠르게 확인 가능:

```bash
cd worker && uv run python -c "import damwha_worker.__main__; print('import ok')"
```
Expected: `import ok` (registry는 `main()` 안에서만 import되므로 이 명령은 heavy 모델을 로드하지 않는다).

- [ ] **(선택) 실 환경 재현 검증** — `uv sync --extra models` 환경에서 정상 워커 1회 실행 시 `process_meeting` + `index_meeting`이 여전히 `committed`되는지(정상 경로 보존 확인). 이번 수정은 정상 경로 동작을 보존하고 실패 경로만 보강한다.
