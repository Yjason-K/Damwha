...
**Tech Stack:** NestJS 10 (TypeScript 5), Node 20+, Postgres 16 + pgvector(`pgvector/pgvector:pg16` 이미지), `pg`(node-postgres) 직접 사용(raw SQL — `SKIP LOCKED`·vector 타입 때문에 ORM 미사용), zod(payload/env 검증), `@nestjs/schedule`(reaper cron), Jest + supertest + `@testcontainers/postgresql`(통합 테스트).
...
be/
  ...
  src/
    ...
    storage/
      storage.module.ts
      storage.service.ts            UUID 키, traversal 가드, save/resolve/stat/stream
...
**Files:**
- Create: `be/package.json`, `be/tsconfig.json`, `be/tsconfig.build.json`, `be/nest-cli.json`, `be/jest.config.js`, `be/.env.example`
- Create: `be/src/config/env.ts`, `be/src/main.ts`, `be/src/app.module.ts`, `be/src/health/health.controller.ts`
- Test: `be/test/health.e2e-spec.ts`
...
Run: `cd be && npm install && npx tsc --noEmit -p tsconfig.json`
Expected: PASS (no type errors). The health e2e test runs in Task 2 once `test/db.ts` exists.

- [ ] **Step 5: Commit**

```bash
git add be/package.json be/tsconfig*.json be/nest-cli.json be/jest.config.js be/.env.example be/src
...
**Files:**
- Create: `be/src/database/database.service.ts`, `be/src/database/database.module.ts`, `be/src/database/migrate.ts`, `be/src/database/migrations/001_init.sql`
- Create: `be/test/db.ts`
...
Run: `cd be && npx jest test/migration.spec.ts`
...
`be/src/database/migrations/001_init.sql`:
```sql
...
> Important: `nest build` (tsc) does not copy `.sql` files to `dist/`. Add to `package.json` `build` script: `"build": "nest build && cp -r src/database/migrations dist/database/migrations"`. For tests, ts-jest runs from `src` so `__dirname` already resolves to `src/database`.

`be/src/database/database.service.ts`:
```ts
...
`be/src/database/database.module.ts` (replaces the stub):
```ts
...
Run: `cd be && npx jest test/migration.spec.ts test/health.e2e-spec.ts`
Expected: PASS (all assertions). pgvector image pulls on first run.

- [ ] **Step 5: Commit**

```bash
git add be/src/database be/test/db.ts be/test/migration.spec.ts be/test/health.e2e-spec.ts be/package.json
...
**Files:**
- Create: `be/src/storage/storage.service.ts`, `be/src/storage/storage.module.ts` (replaces stub)
- Test: `be/test/storage.spec.ts`
...
`be/src/storage/storage.module.ts`:
```ts
import { Global, Module } from '@nestjs/common';
...
```bash
git add be/src/storage be/test/storage.spec.ts
...
**Files:**
- Create: `be/src/contracts/job-payload.schema.ts`
- Test: `be/test/job-payload.spec.ts`
...
Run: `cd be && npx jest test/job-payload.spec.ts`
...
```bash
git add be/src/contracts be/test/job-payload.spec.ts
...
**Files:**
- Create: `be/src/jobs/jobs.types.ts`, `be/src/jobs/jobs.repository.ts`, `be/src/jobs/jobs.module.ts` (replaces stub)
- Test: `be/test/jobs.repository.spec.ts`
...
`be/test/jobs.repository.spec.ts`:
```ts
import { startTestDb, StartedTestDb } from './db';
...
`be/src/jobs/jobs.module.ts` (partial — reaper added in Task 6):
```ts
import { Global, Module } from '@nestjs/common';
...
```bash
git add be/src/jobs be/test/jobs.repository.spec.ts
...
**Files:**
- Modify: `be/src/jobs/jobs.repository.ts` (add `reapStale`)
- Create: `be/src/jobs/reaper.service.ts`
- Modify: `be/src/jobs/jobs.module.ts` (register `ReaperService`)
- Test: `be/test/reaper.spec.ts`
...
`be/src/jobs/jobs.module.ts` (final):
```ts
import { Global, Module } from '@nestjs/common';
import { JobsRepository } from './jobs.repository';
...
```bash
git add be/src/jobs be/test/reaper.spec.ts
...
**Files:**
- Create: `be/src/meetings/meetings.repository.ts`, `be/src/meetings/meetings.service.ts`, `be/src/meetings/meetings.controller.ts`, `be/src/meetings/meetings.module.ts` (replaces stub)
...
describe('meetings', () => {
  ...
  it('GET /meetings/:id returns meeting with ordered utterances', async () => {
    const created = await request(srv()).post('/meetings').attach('audio', Buffer.from('a'), 'a.wav');
...
`be/src/meetings/meetings.service.ts`:
```ts
...
export class MeetingsService {
  ...
  async upload(file: Express.Multer.File | undefined, body: { title?: string; recorded_at?: string }) {
    ...
    if (!AUDIO_MIME.test(file.mimetype)) throw new BadRequestException('file must be audio/*');

    const meetingId = crypto.randomUUID();
    const audioKey = this.storage.meetingKey(meetingId, file.originalname);
    await this.storage.save(audioKey, file.buffer);
...
`be/src/meetings/meetings.controller.ts`:
```ts
import {
...
export class MeetingsController {
  ...
  @UseInterceptors(FileInterceptor('audio', {
    ...
    limits: { fileSize: loadEnv().MAX_UPLOAD_BYTES },
  }))
  upload(
...
`be/src/meetings/meetings.module.ts`:
```ts
import { Module } from '@nestjs/common';
...
```bash
git add be/src/meetings be/test/meetings.e2e-spec.ts
...
**Files:**
- Modify: `be/src/meetings/meetings.controller.ts` (add `GET :id/audio`)
- Modify: `be/src/meetings/meetings.service.ts` (add `getAudioDescriptor`)
- Test: `be/test/audio.e2e-spec.ts`
...
describe('audio streaming', () => {
  ...
  it('serves full body (200) and a byte range (206)', async () => {
    const created = await request(app.getHttpServer())
      .post('/meetings').attach('audio', Buffer.from('0123456789'), 'a.wav');
    const mid = created.body.id;
...
Add to `be/src/meetings/meetings.controller.ts` (imports: add `Headers, Req, Res`; `import { Request, Response } from 'express'`):
```ts
  @Get(':id/audio')
  async audio(
...
```bash
git add be/src/meetings be/test/audio.e2e-spec.ts
...
**Files:**
- Create: `be/src/speakers/speakers.repository.ts`, `be/src/speakers/speakers.service.ts`, `be/src/speakers/speakers.controller.ts`, `be/src/speakers/speakers.module.ts` (replaces stub)
- Test: `be/test/speakers.e2e-spec.ts`
...
describe('speakers', () => {
  ...
  it('POST /speakers creates pending speaker + enroll_speaker job', async () => {
    const res = await request(srv())
      ...
      .attach('audio', Buffer.from('sample'), 'voice.wav');
    expect(res.status).toBe(201);
  ...
  it('POST /speakers requires name and audio', async () => {
    expect((await request(srv()).post('/speakers').attach('audio', Buffer.from('a'), 'a.wav')).status).toBe(400);
    expect((await request(srv()).post('/speakers').field('name', 'x')).status).toBe(400);
  });

  it('GET /speakers/:id returns enrollment_status', async () => {
    const created = await request(srv()).post('/speakers').field('name', 'A').attach('audio', Buffer.from('a'), 'a.wav');
...
Run: `cd be && npx jest test/speakers.e2e-spec.ts`
Expected: FAIL — routes missing.

- [ ] **Step 3: Implement repository, service, controller, module**

`be/src/speakers/speakers.repository.ts`:
```ts
import { Injectable } from '@nestjs/common';
import { Queryable } from '../jobs/jobs.types';
...
`be/src/speakers/speakers.service.ts`:
```ts
...
`be/src/speakers/speakers.controller.ts`:
```ts
import {
...
`be/src/speakers/speakers.module.ts`:
```ts
import { Module } from '@nestjs/common';
...
```bash
git add be/src/speakers be/test/speakers.e2e-spec.ts
...
**Files:**
- Create: `be/src/meetings/clusters.controller.ts`
- Modify: `be/src/meetings/meetings.service.ts` (add `resolveCluster`)
- Modify: `be/src/meetings/meetings.repository.ts` (add cluster/utterance/voiceprint helpers)
- Modify: `be/src/meetings/meetings.module.ts` (register `ClustersController`)
- Test: `be/test/clusters.e2e-spec.ts`
...
Run: `cd be && npx jest test/clusters.e2e-spec.ts`
Expected: FAIL — route missing.

- [ ] **Step 3: Implement repository helpers, service method, controller**

Add to `be/src/meetings/meetings.repository.ts`:
```ts
...
Add to `be/src/meetings/meetings.service.ts` (imports: add `loadEnv` from `../config/env`):
```ts
...
`be/src/meetings/clusters.controller.ts`:
```ts
...
Modify `be/src/meetings/meetings.module.ts` to register the controller:
```ts
import { Module } from '@nestjs/common';
import { MeetingsController } from './meetings.controller';
...
```bash
git add be/src/meetings be/test/clusters.e2e-spec.ts
...
- §7 입력 검증(mime) → Tasks 7,9. 부분 실패 status 컬럼 → Task 2(스키마). (런타임 부분 실패 기록은 Plan 2 워커.)
